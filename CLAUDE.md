...
- **A model column typed pg `UUID` while the real DB column is `varchar` breaks at runtime, and CI can't see it.** The ORM emits `WHERE col = $1::uuid`; Postgres has no `varchar = uuid` operator → `operator does not exist: character varying = uuid` on every query that touches the column. `user_sessions.id` did this in prod and broke auth (session `last_seen_at` writes 500'd). Root cause: commit `2d0ced269` retyped 23 columns `String → UUID(as_uuid=False)` in the models **with no migration**, assuming prod was already uuid — true for 22, but `user_sessions.id` (created `sa.String` by migration 237) was still varchar. **Fresh databases never show it: the app bootstraps with `create_all` from the models (then `alembic stamp head`), so columns are uuid by construction — migrations are never run from scratch (they are incremental patches on a create_all base). The drift exists only on long-lived, migration-built databases (prod).** Fix pattern: an **idempotent** migration that `ALTER … TYPE uuid USING col::uuid` **guarded on the live `data_type`** (no-op where already uuid, fires only on varchar); FK columns drop + re-add their FK around the retype (referenced PKs must be uuid). See migration `301_uuid_column_parity`. To catch it before prod, `tests/test_model_db_type_parity.py` walks every pg-UUID model column and asserts the connected DB agrees — but it is **only meaningful pointed at a production mirror**; against CI's create_all DB it is a trivial pass. The model side also flip-flops (`user_sessions.id`: String `59576f097` → UUID `2d0ced269`), so the same test pins the 23 columns as pg UUID in the models.
- **Alembic `alembic_version.version_num` is `varchar(32)`.** The fresh-DB `alembic stamp head` inserts the head revision id directly, so a revision id > 32 chars (`300_polymorphic_source_entity_id_uuid` = 37) fails the bootstrap with `value too long for type character varying(32)`. Only the HEAD id length matters for `stamp`; mid-history long ids are never re-inserted. Keep revision ids ≤ 32 chars.
- **Schema-introspecting tests are polluted by throwaway tables other test modules map on the shared `Base.metadata`.** A guard that walks `Base.metadata.tables` (e.g. `test_schema_uuid_id_principle`) passes in isolation but fails in a full-suite run when `test_lookup_binding_service` (`demo_widgets`) / `test_lookup_write_enforcement` (`fake_lookup_target`) have registered their models. Attribute each mapped table to its defining module via `Base.registry.mappers` and drop the ones under `tests.` before judging.
- **A long-running `uvicorn --reload` dev server can serve STALE code after a `git` operation.** After models/services were edited + committed, `--reload` did not pick them up; the running process kept an old `notification_service` that queried the now-uuid `notifications.source_entity_id` with a synthetic string, 500ing and aborting the shared transaction — which then cascaded into unrelated form-submit requests. In-process (fresh interpreter) worked; only the live server failed. When cross-cutting 500s appear that don't reproduce in a REPL, restart the dev server before deeper debugging.
- **Contact-impersonation portal tokens are origin-scoped.** `POST /user-management/contact-impersonation/start` returns a `portal_url` built from `FRONTEND_BASE_URL`; the landing writes the token to **that origin's** `sessionStorage` (impersonation tokens are session-scoped, not localStorage). Navigating a test/browser to a different origin (e.g. `localhost:3000`) and then to a portal path finds no token → every portal fetch 401s. Extract the token from `portal_url` and navigate the SAME origin. Impersonation also stays on the legacy `/portal` tree (Welcome header, `/portal/{type}/new`), not `/portal/c/{slug}`.
