...
- **Chat latency needs `respond_ts` on BOTH rows; `turn_id` only pairs them.** `chat_history_query` computes latency from `respond_ts` alone — `sent_at` is n8n's save clock and on real rows an outgoing `sent_at` can precede the incoming it answers. So `turn_id` flowing in correctly still yields "—" in the Latency AND Delivery columns if `respond_ts` is NULL. Symptom looks like a pairing bug; it's a clock bug.
- **Respond's `messageId` IS the message's epoch-MICROSECOND timestamp.** So `respond_ts` is derivable at ingest with zero HTTP — no resolver round trip, no single point of failure on the SLA clock. Inbound WhatsApp ids land on exact whole seconds (trailing `000000`); that granularity is Respond's, and it's the tell that the id is genuinely theirs and not something n8n computed. **Do NOT parse it with `_epoch_to_naive_utc`** — that helper reads anything `>1e12` as milliseconds, turning a µs id into the year 58,000 (the trap its own docstring warns about). Use `respond_ts_from_message_id`, and keep its ±1-day cross-check against `sent_at`: a short sequence-style id (`1234556`) otherwise resolves to 1970 and injects a 56-year round trip into the p99.
- **A silent exception branch makes a broken background job indistinguishable from one that never ran.** `chat_message_resolver`'s 404 path incremented `resolve_attempts` and `continue`d without logging, so a resolver 404-looping on every row looked exactly like a resolver that was never scheduled. Non-zero `resolve_attempts` on rows was the only evidence it ran at all. Every `except ... : continue` in a scheduled task needs a log line.
- **`yield_per` + a mid-loop `commit()` = `psycopg2.ProgrammingError: named cursor isn't valid anymore`.** `yield_per` opens a server-side (named) cursor; committing closes the transaction that owns it and the next `fetchmany` dies. Streaming and committing are mutually exclusive. Page batch scripts with **keyset** (`WHERE id > :last_id LIMIT :n`) instead — and keyset not OFFSET, because when the script's own writes remove rows from its filter (`WHERE respond_ts IS NULL`), an OFFSET skips forward over unprocessed rows by exactly as many as it just fixed, silently half-finishing while reporting success. Note a `--dry-run` will NOT reproduce this (it never commits), and a sqlite test can't either (no server-side cursors) — verify batch scripts against Postgres at `--batch 1`.
- **`--dry-run` must assign nothing, not just skip the commit.** Setting attributes on ORM instances makes autoflush push UPDATEs into the transaction ahead of the next SELECT, so a "report only" run issues writes that only a final rollback undoes.
