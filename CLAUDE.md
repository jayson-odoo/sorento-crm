...
- **A new migration's `down_revision` must be the real chain TIP, found via `alembic`'s `ScriptDirectory.get_heads()` — NOT the highest-numbered file.** Migration file numbers are not chain order. This repo uses **merge migrations** (`down_revision` is a tuple, e.g. `9785e8947154` = `('273_import_source_file', '285_market_signal_sources')`) that fold a numbered node into another chain. So a high-numbered file like `285_market_signal_sources` can already be *consumed* — its real descendant was the ideation chain `272 → 273 → 274_ideation_workspace_config`. Chaining a new migration onto a consumed node forks it → **two heads** → the deploy entrypoint's `alembic upgrade head` (singular) fails hard: `FAILED: Multiple head revisions are present for given argument 'head'` (this broke #19's blue/green deploy; fixed in #20 by re-chaining `286` onto `274_ideation_workspace_config`). **Before setting `down_revision`:** run `ScriptDirectory.from_config(Config("alembic.ini")).get_heads()` against the merge-target tree — needs dummy `DATABASE_URL`/`JWT_SECRET`/`SECRET_KEY` env so the app-importing seed migrations load — and assert exactly ONE head; chain onto it. Never regex-parse `down_revision` strings to find heads: that misses tuple/merge parents and over-counts. If the target genuinely has 2 legit heads, add a merge migration instead.
