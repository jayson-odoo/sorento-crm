...
* Then click one of:
  * **Create link only** — generates the link without sending an email. Useful when you want to share the link via another channel (e.g. paste into chat).
  * **Create link & send email** — generates the link **and** emails it to the approver address.
