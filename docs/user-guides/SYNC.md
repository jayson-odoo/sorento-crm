...
The user guides under `docs/user-guides/` are mirrored to the **Sorento CRM** collection on Outline (`https://doc.foundryx.my/collection/sorento-crm-BOoUtlxxTH`). MCP tools, the in-app footer "Docs" link, and the n8n / WhatsApp agent all read from Outline at runtime, so Outline is the user-facing source of truth.
...
State (path → Outline doc id) is stored in `docs/user-guides/.outline-sync.json`. Commit it. Without state, the script falls back to title matching.
