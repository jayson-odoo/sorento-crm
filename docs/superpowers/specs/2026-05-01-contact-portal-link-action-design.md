...
- Sending the link via Respond.io chat (deferred)
...
4. Modal displays: link in copyable input, expiry, "Open in new tab" link, QR code, optional "Existing valid link" badge if a live token was reused.
5. Admin copies / opens / scans QR. Modal closes.
...
- `sorento_crm_backend/app/api/v1/user_management/contacts.py` — new route handler
- `sorento_crm_backend/app/services/portal_service.py` — add `get_or_mint_token`
- `sorento_crm_backend/tests/` — unit tests for service reuse logic + endpoint RBAC + 422 missing workspace
...
- `sorento_crm_frontend/services/contactPortalLinkService.ts` — `getContactPortalLink(contactId): Promise<PortalLinkResponse>` calling `POST /user-management/contacts/{id}/portal-link` via shared `apiFetch` + `extractApiError`.
- `sorento_crm_frontend/hooks/useContactPortalLinkMutation.ts` — react-query `useMutation` wrapping the service, with toast on error.
...
Frontend:
- Vitest: `PortalLinkDialog` renders link + QR on success; renders "Reused existing link" badge when `reused=true`; copy button calls clipboard API.
- Manual: click action on contact detail, list row, SLA tracking detail. Confirm three places work and dialog identical.
