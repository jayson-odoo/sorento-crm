...
**Architecture:** Phase 2 of `docs/superpowers/specs/2026-05-01-mcp-tool-access-guard-design.md` (§9, §10). Single transaction PUT semantics: claim selected tools (`UPDATE mcp_tools SET agent_id=:id WHERE id IN :tool_ids`) and release deselected (`UPDATE ... SET agent_id=NULL WHERE agent_id=:id AND id NOT IN :tool_ids`). UI exposes `current_agent_*` fields on the picker so admins see ownership conflicts before saving.
