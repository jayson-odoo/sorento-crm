...
":" or spaces) is `board_thickness`; a number after "盆胆" (bowl) is `thickness`;
"厚度 N" or a bare "N" / "Nmm" is also `thickness` (the sheet is naming the bowl the
only way it can when there is no drainer board to distinguish it from). Anything
else is a parse failure - the row is reported and skipped, never guessed.
...
to SRTKS7850-2. A code appearing twice in one sheet with IDENTICAL parsed values is
written once; twice with DIFFERENT values is a conflict, reported and skipped
entirely (neither instance is written).

REGISTRY KEYS
-------------
Two keys are created if missing, mirroring `create_spec_key`
(`app/api/v1/master_data/spec_registry.py`) - `source="user"`, `synonyms={}`,
`match_tolerance`/`match_decay` from `default_match_window(unit)`:
  ...
  * `surface_texture` - enum (normal / honeycomb / nano_volcano / nano_grain /
    ...
    `rank_weight` as the seeded `finish` key.
...
SAFETY
------
Default is DRY RUN: the whole run - registry creation AND every
...
def parse_thickness_text(text: str) -> dict[str, float]:
    ...
    "0.9mm", "0.9mm " (trailing space), "1.15mm", "2.5mm". Anything else raises
    ThicknessParseError.
    """
...
def parse_sheet(ws, layout: SheetLayout) -> list[RowResult]:
    data_rows = [
    ...
    for r in data_rows:
        ...
        if failures:
            results.append(RowResult(row=r, code=code, parse_failure="; ".join(failures)))
            continue

        if not entries:
            results.append(RowResult(row=r, code=code, blank=True))
            continue

        results.append(RowResult(row=r, code=code, entries=entries))
    return results


def dedupe_rows(
    rows: list[RowResult],
) -> tuple[dict[str, dict], dict[str, int], list[tuple[str, list[int]]]]:
    """to_write (code -> entries), first_row (code -> row, for evidence), conflicts.

    A code seen twice with the IDENTICAL parsed entries is written once. Twice
    with DIFFERENT entries is a conflict - reported, and neither instance writes.
    """
    ...
    for code, group in by_code.items():
        ...
        if all(g.entries == first for g in group):
            to_write[code] = first
            first_row[code] = group[0].row
        else:
            conflicts.append((code, [g.row for g in group]))
    return to_write, first_row, conflicts
...
def ensure_registry_keys(db: Session, *, apply: bool) -> dict:
    ...
    Never touches a key that already exists.
    """
...
def _registry_row(db: Session, spec_key: str):
    """The live registry row, or - dry run only, before `--apply` has created it -
    a transient, never-added instance carrying the shape `--apply` WOULD create,
    so validation sees the same vocabulary either way.
    """
    from app.models.product_spec import ProductSpecRegistry
    ...
    definition = _NEW_KEY_DEFS.get(spec_key)
    if definition is None:
        raise SystemExit(
            f"no registry row for {spec_key!r}, and it is not one of the keys this "
            f"script creates - run seed_spec_registry first."
        )
...
def run(db: Session, xlsx_path: str, *, apply: bool, only_sheet: Optional[str] = None) -> dict:
    ...
    registry_report = ensure_registry_keys(db, apply=apply)

    required_keys = ["thickness", "material", "steel_grade"]
    missing = [k for k in required_keys if _registry_row(db, k) is None]
    if missing:
        raise SystemExit(
            f"registry is missing required key(s) this loader does not create: "
            f"{', '.join(missing)} - run seed_spec_registry first."
        )
    ...
    for sheet_name in wb.sheetnames:
        ...
        ws = wb[sheet_name]
        layout = SHEET_LAYOUTS[key]
        rows = parse_sheet(ws, layout)
        ...
        to_write, first_row, conflicts = dedupe_rows(rows)
        ...
        for code, spec_values in to_write.items():
            ...
            try:
                entries = _entries_for(db, key, first_row[code], spec_values)
                apply_spec_values(
                    db,
                    db_code,
                    entries,
...
def _print_report(report: dict) -> None:
    ...
    for s in report["sheets"]:
        ...
        if s.unmatched:
            print(f"unmatched codes: {', '.join(s.unmatched)}")
        if s.blank_rows:
        ...
        if s.conflicts:
            print("conflicts:")
            for code, conflict_rows in s.conflicts:
                print(f"  {code}: rows {conflict_rows} disagree")
        if s.write_failures:
