...
def seed_chatbot_policy() -> None:
    ...
    `chatbot_rearch_s6e` / `chatbot_rearch_s7` update four domains' narrowing. All five
    are migration-BODY work: `create_all` gives a bootstrapped database the TABLES and
    COLUMNS (every one is a plain model default) but none of the rows these migrations
    INSERT/UPDATE, so a fresh CI database has empty policy tables and no published
    parser version at all.

    Each migration's own function is imported and called directly, in the same order
    the real migration chain applies them (s0 -> s4 -> s6d -> s6e -> s7 - s4's publish
    reads whatever `chatbot_domains` holds at the moment it runs, same as a real
