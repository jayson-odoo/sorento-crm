"""`scm.committed_v` never caps an ORDER_BACK row's owed quantity

Revision ID: 525_committed_v_orderback
Revises: spec_vocab_close_couple
Create Date: 2026-09-22 00:00:00.000000

Owner ruling 22 Sep 2026 (SO417310 / MKT5529SS-DIY): an ORDER_BACK row is a hole at the
DONOR location left behind when goods already shipped off the borrowing line, so the
borrowing line reading delivered in full is the NORMAL case for an order back, not a
reason to zero it out. Both project legs' owed-quantity expression (`app.services.scm.
demand._OWED_SQL` / `_OWED_FORM_SQL`) gain a `CASE WHEN oir.verb = 'ORDER_BACK' THEN
oir.qty ELSE <511/512's cap> END` branch ahead of the existing cap; an ORDER row keeps
the 14 Sep cap (SO368872 / SRTWC286-SH) unchanged.
...
512's body is frozen beside it for the downgrade. `CREATE OR REPLACE` is legal: same
columns, same order, same names, only the two project legs' owed-quantity CASE expressions
gain one WHEN each.
"""
from alembic import op

revision = "525_committed_v_orderback"
