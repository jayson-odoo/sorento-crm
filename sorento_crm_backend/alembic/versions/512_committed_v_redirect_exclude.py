...
only coverage had already shipped to another order carries no demand any more - its `qty`
and its links are history, kept on the row for the record, and the fresh need is a
separate row this view already counts through the usual confirmed/form leg. Both project
legs over `projects.order_inquiry_rows` (confirmed and form) gain `AND oir.
redirected_to_pool = FALSE`, otherwise identical to 511.

The confirmed leg is not strictly reached by this today - a redirect happens inside a
settle, and the settle's own confirmation supersedes the row's PRIOR supply decision in
the same transaction, so `d.state = 'active'` already drops it. The FORM leg has no such
side effect (`supply_decision_id IS NULL` rows carry no decision to supersede), so without
this clause a redirected row raised through the CS Order Inquiry Form would go on netting
its old, already-shipped quantity forever. Both legs get the same clause so a reader of
this view never has to know which path raised a row before trusting what it says.

The body is FROZEN here rather than imported from `app.services.scm.demand.
COMMITTED_V_SQL` (`test_committed_v_migration_chain.py`'s own drift guard), and 511's body
is frozen beside it for the downgrade. `CREATE OR REPLACE` is legal: same columns, same
order, same names, only the two project legs' WHERE clauses gain one condition each.
"""
...
WITH legs AS (
    ...
    --
    -- A REDIRECTED row is excluded (PLAN-oi-replan-received-links.md S2/S3, AC-RL-15):
    -- its coverage already shipped to another order, and the fresh need it left behind is
    -- a separate row.
    SELECT sol.product_id,
    ...
    --
    -- A REDIRECTED row is excluded here too - this leg has no `d.state = 'active'` join
    -- to drop it as a side effect the confirmed leg gets for free (AC-RL-15).
