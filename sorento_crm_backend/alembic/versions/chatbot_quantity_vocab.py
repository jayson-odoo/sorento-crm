"""Publish the chatbot parser prompt with the new per-entity `quantity` key as a
NEW version, label unmoved (issue #1262, Samantha case, slice 5, finding F4).
...
DB row exists at all), so a session must have a published version carrying
`QUANTITY_ADDENDUM` before it can reach a live turn.

Owner ruling 1 (issue comment 26 Sep ~06:40Z, binding): "i don't want hard code, the
parser supposed to be able to identify the quantity right?" - the schema gains a
per-entity `quantity` (`app/services/chatbot/head/parser.py`, code change, not this
migration's business) and this addendum teaches the model the shape a quantity
actually takes beside a product code ("xN", "N pcs", "N units") and that a
caption-only quantity line applies to the photo's own products - never a rule for
the CODE to strip a quantity back out of text it already read correctly.

Both bodies are published for the measured reason 487 / 490 / 514 / 517 / 519 / 520
/ 521 give: prod's `production` label sits on the FULL `SEMANTIC_PARSER_PROMPT`,
...
Group C (the brand feed, same issue) amends the SAME prompt for its own `Known
brands:` line - designed to land as a SECOND, later migration stacking after this
one (`QUANTITY_ADDENDUM` then a brand addendum), not folded into this one, so each
slice's prompt change promotes independently.

Revision ID: chatbot_quantity_vocab
...
revision = "chatbot_quantity_vocab"
...
def _publish_one(session: Session, template: str, tag: str) -> int | None:
    ...
    if existing is not None:
        logger.info(
            "chatbot parser quantity-vocab %s prompt already published as v%s; "
            "nothing to do",
            tag,
            existing.version,
        )
    ...
    session.commit()
    logger.info(
        "published chatbot parser quantity-vocab %s prompt as v%s (%s chars); "
