"""Publish the chatbot parser prompt with the sales-report vocabulary as NEW
versions, label unmoved (PLAN-chatbot-sales-report.md S4 wiring point 1).

Same reason `514_chatbot_outstanding_vocab` publishes: `ai_prompt_registry.render()`
...
`order_status: "sales_report"` and the new `sales_channel` output key before either
can reach a live turn, however the message is phrased.

Both bodies are published for the measured reason 487 / 490 / 514 give: prod's
`production` label sits on v1 = the FULL `SEMANTIC_PARSER_PROMPT`, while the local /
dev label sits on the SLIM one, so publishing one text reaches one deployment only.
...
Revision ID: 519_chatbot_sales_report_vocab
...
revision = "519_chatbot_sales_report_vocab"
...
def _publish_one(session: Session, template: str, tag: str) -> int | None:
    ...
    if existing is not None:
        logger.info(
            "chatbot parser sales-report-vocabulary %s prompt already published as v%s; "
            "nothing to do",
            tag,
            existing.version,
        )
    ...
    session.commit()
    logger.info(
        "published chatbot parser sales-report-vocabulary %s prompt as v%s (%s chars); "
