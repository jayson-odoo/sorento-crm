"""Publish the chatbot parser prompt with the S19 third-round wording as NEW
versions, label unmoved (PLAN-chatbot-sales-report.md, live testing 19 Sep 2026,
two more owner findings after 520 shipped).

Same reason `520_sales_report_hint_fix` publishes: `ai_prompt_registry.render()`
...
amended `SALES_REPORT_ADDENDUM` wording before it can reach a live turn. Two fixes
folded into one migration, both against `SALES_REPORT_ADDENDUM`:

* Finding 2: "dealer Srt5674 August total sale quantity" answered with the S18
  all-time default instead of an August window - a bare month name beside a channel
  word and a product code was silently dropped. The addendum now states, and gives
  varied examples of, "a bare month name anywhere in a sales report ask is a DATE
  FILTER for that month" - DATE FILTER's own domain-independent rule, restated at
  the one seam that measurably lost it - and rewords the one existing example whose
  SILENCE on date_filter_start/end (it only ever showed sales_channel and the
  entity) could itself have taught "channel word + code => no date".
* Finding 3(a): "Srt5674 August total sale quantity" (no "dealer") sometimes came
  back `domain_hint: "master_products"` alongside a correct `order_status:
  "sales_report"` - the parser knew the ask was a report and still named the wrong
  domain. The addendum now states a sales report ask ALWAYS carries domain_hint
  "order" / intent_hint "check_order", the same pairing every other order_status
  value already uses, and that a product code alone never makes it a product-master
  ask. (Finding 3(b), the deterministic code-side correction that forces domain_hint
  back to "order" whenever `order_status` is a report status, needs no prompt text
  and ships in `head/output_exchange.py` instead - this migration is prompt-only.)

Both bodies are published for the measured reason 487 / 490 / 514 / 517 / 519 / 520
give: prod's `production` label sits on the FULL `SEMANTIC_PARSER_PROMPT`, while the
...
Revision ID: 521_sales_report_month_fix
Revises: 520_sales_report_hint_fix
"""
import logging
...
revision = "521_sales_report_month_fix"
down_revision = "520_sales_report_hint_fix"
branch_labels = None
depends_on = None
...
def _publish_one(session: Session, template: str, tag: str) -> int | None:
    ...
    if existing is not None:
        logger.info(
            "chatbot parser sales-report-month-fix %s prompt already published as v%s; "
            "nothing to do",
            tag,
            existing.version,
        )
    ...
    session.commit()
    logger.info(
        "published chatbot parser sales-report-month-fix %s prompt as v%s (%s chars); "
