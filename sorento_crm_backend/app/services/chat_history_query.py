...
def _turn_latencies(db: Session, rows: list[ChatHistory]) -> dict[str, float]:
    """Latency per turn for the turns present in `rows`.
...
def list_messages(
    ...
    *,
    ...
    """Newest-first page of messages plus the cursor for the next page."""
    ...
    for row in rows:
        ...
        out.append(
            ChatMessageRow(
                ...
                turn_id=row.turn_id,
                message_id=row.message_id,
                latency_seconds=latencies.get(key) if (key and row.type == "outgoing") else None,
