...
TOOL_INTENTS: dict[str, ToolIntent] = {
    ...
    "crm_forms_complaints_submit": ToolIntent(
        ...
        description=(
            ...
            "supplied, it returns status `needs_do_lookup` and next_action `collect_do_lookup_filters`. Those filters "
            "(customer_name, product_code/product, order_date_from, order_date_to) are ALL OPTIONAL — ask the user for "
            "any one of them (combining narrows results); do not demand all four. Relay that DO lookup request; "
