...
def _last_po_supplier_map(db: Session, product_ids: list[str]) -> dict[str, dict]:
    ...
    which is the buyer's own reading of "Supplier". A cancelled PO does not exist for
    this lookup (PLAN-product-supplier-all-po.md ruling 3, 10 Sep 2026) - it can never be
    the "newest" PO that names the Supplier column, so this stays in agreement with
