...
class Focus(BaseModel):
    ...
    # D21 (PLAN-chatbot-dealer-stock-verdict.md): the open tasks, carried INSIDE the
    # focus rather than on a session key of their own. Declared here because this model
    # is `extra="forbid"` and `turn/state.py::focus_to_wire` writes the key on every
    # turn - a shape the session validator did not know would fail the write itself.
