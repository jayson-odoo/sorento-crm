...
def _spec_quantities(
    ...
    Two sources, one shape, and this is the one seam where both halves exist. The OPEN
    TASK's own slots win where a task drove the fetch (`turn/task.py::StockQtyTask.
    to_fetch` stamps them on the spec) - they carry quantities this message never
    repeated, which is the whole point of the task. Otherwise it is the quantities THIS
    message stated per entity, joined by CODE to the uuid the resolver placed: the first
    turn of a stock ask has no task yet, because nothing has told the engine a quantity
    is required until the reply says so (D25).
    """
    ...
    *,
    ...
    def runner(domain: str, spec: FetchSpec) -> dict[str, Any]:
        ...
        if page_predicate is not None:
            ...
            # An OPEN TASK's own fetch is about the TASK's subjects, all of them
            # (`turn/task.py::StockQtyTask.to_fetch`): a turn answering two of four
            # products resolves only those two, and `_entities_for` below would keep
            # exactly the resolver's two - so the answer would silently drop the two
            # the dealer had already given a quantity for. The task is the only honest
            # record of what the question is about.
        ...
        # D13/D20: the dealer's own quantity per product, resolved to uuids here -
        # `lanes/business/fetch.py` reads it straight off the lane input.
...
def envelope_of(
    ...
    *,
    ...
    envelope: dict[str, Any] = {
        ...
        # D25: what the stock reply said about a quantity being required, per product.
        # The composer never reads it (the sentence is the presenter's); `engine.py`
        # rebuilds the open stock task from it (`turn/task.py::tasks_after_reply`).
        "stock_availability": (
            fetched.get("stock_availability")
            if isinstance(fetched.get("stock_availability"), list)
            else []
        ),
        # D15: the products a "just proceed" dropped, named by the reply so the dealer
        # can see what was not checked. Stamped on the spec by the task, carried here
        # because the composer prints it under the section it belongs to.
        "not_checked": [
