...
class AIAssistantChatService:
    ...
    def _default_system_prompt(self) -> str:
        return (
            ...
            "3) NEVER call the submit tool until (a) every REQUIRED header field is "
            ...
            "4) When you do submit, pass ONE argument `payload_json` shaped per the "
            "tool's docstring. After success, briefly acknowledge the submission "
            "and (for complaints) offer to attach photos/videos via "
            "`crm_forms_entity_attachments_link`.\n"
            "5) If the user says 'new <form>', 'start over', or changes to a "
            "different form, clear the in-memory form state and restart at step 1.\n"
            "6) For complaint forms only: BEFORE collecting complaint fields, make "
