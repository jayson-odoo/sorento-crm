"""Low-stock-report startup bootstrap (S6, PLAN-low-stock-report.md, AC-65).

`outstanding_report_bootstrap` under a different name - same module, same three things
worth pinning (it appends, it is idempotent, it does not raise when there is nothing to
append to). Runs after ``sync_catalog`` so the ``crm_low_stock_report`` row exists in
...
The chatbot lane is a DIFFERENT consumer - it picks tools from its own
DOMAIN_SPEC pools, not this list (S6 wires it there separately). This
bootstrap only concerns the in-app assistant.
...
TOOL_NAME = "crm_low_stock_report"
...
def run(db: Session) -> dict:
    ...
    try:
        ...
        logger.warning("Low stock report bootstrap: AI assistant enabled_tools update failed: %s", e)
        try:
            db.rollback()
        except Exception:  # noqa: BLE001
            pass
    logger.info("Low stock report bootstrap finished: %s", summary)
    return summary
...
def _enable_tool_for_ai_assistant(db: Session) -> bool:
    ...
    if not config:
        logger.info("Low stock report bootstrap: AIAssistantConfig row missing; skipping enable")
        return False
    ...
    db.commit()
    logger.info(
        "Low stock report bootstrap: appended %s to AIAssistantConfig.enabled_tools",
        TOOL_NAME,
    )
    return True
