...
# Two aliases of the warehouse TABLE and two of the user table, so one row read names both
...
def cancel_open_for_decision(db: Session, decision_id: str, reason: str) -> int:
    """Call off the open transfers of a superseded revision.
    ...
    rows = (
        ...
        .filter(
            StockTransfer.supply_decision_id == decision_id,
    ...
    for snapshot in snapshots:
        ...
        for component in snapshot.get("components") or []:
            ...
            if not from_warehouse_id or not to_warehouse_id or not product_id:
                ...
                logger.warning(
                    "stock transfer not written for decision %s: from=%s to=%s product=%s",
                    decision.id,
                    source_location,
                    own_location,
                    product_id,
...
class StockTransferService:
    ...
    def bulk_approve(self, ids: Iterable[str], *, actor_user_id: str) -> Dict[str, Any]:
        approved = 0
