...
Only the parser is stubbed, the same `Console` the round 4 and 5 replays drive. A
message the fast path reads (`head/fast_path.py`: a bare number or numbered lines over
the open stock question) is read by it, exactly as `engine.run_turn` does, and never
...
def _read(console: Console, message: str) -> dict:
    """What the engine hands `apply` for a message the fast path reads."""
    read = fast_path.read(message, console.state)
    assert read is not None, f"the fast path did not read {message!r}"
    assert read.kind == "stock_quantity"
    return read.verdict


def _say_fast(console: Console, message: str) -> str:
    return console.say(message, _read(console, message))
...
def test_per_line_reply_fills_those_lines_and_asks_the_rest_in_point_form():
    console = _after_all()
    text = _say_fast(console, "1. 10, 2. 5")
...
def test_every_line_answered_at_once_answers_the_check():
    ...
    assert _say_fast(console, message) == _answered(
...
def test_the_dealer_may_paste_the_lines_back_filled_in():
    ...
    text = _say_fast(console, message)
...
def test_the_full_owner_transcript_after_the_fix():
    console = _after_all()
    _say_fast(console, "1. 10, 2. 5")
    _say_fast(console, "10")
...
def test_round_five_one_product_pick_then_numbers_still_revise():
    ...
    assert _say_fast(console, "10") == f"SRTWC286-SH x 10: {TOO_BIG}"
    assert _say_fast(console, "2") == f"SRTWC286-SH x 2: {TOO_BIG}"
    assert console.say("3", verdict(reference_positions=[3], entities=[])) == (
        f"SRTWC286-SH x 3: {TOO_BIG}"
    )


def test_the_fast_path_never_reads_under_an_open_pick():
    """While the which-one list is open a number is a pick (round 4), so the parser and
    `decide()` read it, never the fast path."""
    console = Console()
    console.first_ask("check stock srtwc286")
    assert fast_path.read("2", console.state) is None
    assert fast_path.read("1. 10, 2. 5", console.state) is None
