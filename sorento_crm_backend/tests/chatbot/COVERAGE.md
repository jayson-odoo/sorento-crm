...
A branch under the bar in a pool that was scanned END TO END is `exhausted`, not short: the traffic does not exist, so no further capturing produces it and gate 0 must not block on it. `scanned` counts the live executions ON THE CURRENT WORKFLOW VERSION, all examined; the rest of `pool` ran on older versions and cannot be graded against the body the export ships.

| workflow | version | executions scanned | pool | captured | nodes |
| --- | --- | ---: | ---: | --- | --- |
| `spine-rs-1a` | `51f7b0d2` | 567 | 946 | 2026-09-04 | `route-turn`, `build-ctx` |
| `sub-semantic-parser` | `ab3ec985` | 239 | 3901 | 2026-09-04 | `output_exchange`, `suggest-follow-up` |
