...
class TestBothPublishedBodiesCarryTheVocabulary:
    def test_the_addendum_is_appended_to_both_bodies(self) -> None:
        ...
        from app.services.chatbot_parser_prompt import SALES_REPORT_ADDENDUM, TOP_SELLING_ADDENDUM
        ...
        for name, body in _bodies().items():
            assert body.removesuffix(TOP_SELLING_ADDENDUM).removesuffix(SALES_REPORT_ADDENDUM).endswith(addendum), (
