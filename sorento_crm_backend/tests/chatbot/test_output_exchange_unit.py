...
def _report_status_domain_out(*, domain_hint, order_status) -> dict:
    """`post_process`'s output for an emission with the given `domain_hint` /
    `order_status`, a single product entity, nothing else about the turn varied."""
    json_item = {
        "output": json.dumps(
            _parser_output(
                ...
                requested_attributes=["quantity"] if order_status == "sales_report" else [],
                entities=[
                    {
                        "raw": "SRT5674",
                        "hint": "product",
    ...
    parent_input = {
        "latest_user_message": "Srt5674 August total sale quantity",
        "contact_id": "ZZT-report-domain-1",
        "previous_conversation_state": {},
    }
    parsed = output_exchange(json_item, parent_input)
...
class TestReportOrderStatusAlwaysRoutesToOrderDomain:
    ...
    the measured emission for "Srt5674 August total sale quantity" parsed
    `order_status: "sales_report"` alongside `domain_hint: "master_products"` - the
    parser knew the ask was a report and still named the wrong domain, and
    `lanes/business/__init__.py`'s tool-pick override for `sales_report` (like the
    outstanding one beside it) only fires when `domain == "order"`, so the turn fell
    through to the product-master listing instead of the report. Read ONLY the
    parser's own structured `order_status` field, never the message text."""

    def test_master_products_domain_with_sales_report_status_is_corrected_to_order(
        self,
    ) -> None:
        out = _report_status_domain_out(domain_hint="master_products", order_status="sales_report")
        assert out["domain_hint"] == "order", out
        assert out.get("domain_corrected") == "master_products->order (order_status sales_report)", out
    ...
    def test_a_non_order_domain_with_an_outstanding_status_is_also_corrected(
        ...
        out = _report_status_domain_out(domain_hint="inventory", order_status=order_status)
        assert out["domain_hint"] == "order", out
    ...
    def test_domain_hint_order_already_is_left_untouched(self) -> None:
        out = _report_status_domain_out(domain_hint="order", order_status="sales_report")
        assert out["domain_hint"] == "order", out
    ...
    def test_a_null_order_status_with_master_products_domain_is_left_alone(self) -> None:
        ...
        out = _report_status_domain_out(domain_hint="master_products", order_status=None)
