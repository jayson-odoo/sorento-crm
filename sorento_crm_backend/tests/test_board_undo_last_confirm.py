...
# (journal path, no prior decision). Owner hand test 18 Sep found nine lines      #
# still `Confirmed` on the board with zero decisions left in the DB, on an order  #
# shaped like SO314594: pre-existing OI rows sheet-migrated in (placed /          #
# partly_linked / actioned, no `supply_decision_id`) alongside a line a real      #
# Confirm's own journalled decision covered. TEST-FIRST against today's board     #
# read, which still marks the undone line `covered: true` - see `test_board_     #
# undo_reconstructed.py::test_no_confirmed_verdict_after_reconstructed_undo_     #
# with_no_prior` for the reconstructed twin.                                     #
# =============================================================================== #


def test_no_confirmed_verdict_after_journal_undo_with_no_prior(api):
    """AC-R2-19. A single real Confirm (via `POST .../confirm`, journalled, the
    order's first and only revision) is undone via `undo_last_confirm`. The board
    read for that order must then show every line undecided (`covered = False`,
    `decision = None`), the order's own `undo` gone, and - as a control that the
    ...
    from app.models.project_so import INQUIRY_ACTIONED, INQUIRY_PARTLY_LINKED, INQUIRY_RAISED
    from app.services.project_supply_undo_service import undo_last_confirm
    ...
    # SO314594's own shape: pre-existing OI rows sheet-migrated onto the order BEFORE
    # any board decision ever touched it, no `supply_decision_id` at all, under the
    # ONE header `order_inquiries` permits per order (`uq_project_order_inquiry_per_
    # sales_order`) - so a later Confirm's own raise lands on this SAME header, exactly
    # as it does in prod, rather than a second one. Seeded BEFORE the confirm so the
    # journal it writes never captures these rows as ITS OWN insert - they predate it,
    # wholly independent of the decision that gets undone.
    inquiry = OrderInquiry(
    ...
    by_so = {r["so_number"]: r for r in payload["orders"]}
