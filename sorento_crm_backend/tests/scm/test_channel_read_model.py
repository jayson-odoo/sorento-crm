...
def _confirmed_leg(db, *, product_id, warehouse_id, buy_qty, decision_state="active",
    ...
    `SOSupplyDecision` and `OrderInquiryRow.supply_decision_id` were new when this file was
    written (STAGE2-... worknotes section 0.1) - this helper's construction of them is
    exactly what makes every caller fail with an ImportError / TypeError until the coder
    adds them, which is the red this whole confirmed-leg half of the file is pinning. Both
    now come from Stage 1C, which owns the model and the migration; Stage 2 only reads
