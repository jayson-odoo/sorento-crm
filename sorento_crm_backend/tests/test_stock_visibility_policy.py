...
def test_availability_pages_over_products(db):
    ...
    D35 (review round 11) answers the "nobody asked about" half at the source - a dealer
    ask that names NO product now comes back as an empty block and a request for the
    code (`test_availability_with_no_product_named_asks_for_the_code`, the availability
    block's own file), never as a page. So the products are named here, and what this
    still pins is the cap itself: a named set larger than the page returns one page of
    it with the true total beside it."""
    brw, _, _ = _three_warehouses(db)
    products = []
