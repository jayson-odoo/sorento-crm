...
def test_neighbours_out_of_filter_falls_back_to_unfiltered(db: Session) -> None:
    ...
    # totals of the real, shared `access_agents` table for equality. That table
    # is not this test's own - it is the whole suite's, and another xdist
    ...
    fallback_ids = {
        str(row[0]) for row in svc._build_list_query().with_entities(AccessAgent.id).all()
    }
