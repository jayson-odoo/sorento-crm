 ...
 *
 * The trigger states a MINIMUM height, not a fixed one, and never clamps its label
 * to one line: a chosen option has to be readable in full. A supplier picker showing
 * "CHAOZHOU JINBAICHUAN SANITARY WARE TECHNOLO..." tells the reader nothing about
 * which of two similarly-named suppliers the plan was built for, and the control
 * cannot always be widened to suit the longest name in the list. Long labels wrap
 * and the control grows; every size keeps its one-line height when nothing wraps.
 */
export const selectTriggerVariants = cva(
  ...
  `,
  {
    variants: {
      size: {
        sm: 'min-h-7 px-2.5 py-0.5 text-xs gap-1 rounded-md',
        md: 'min-h-8.5 px-3 py-1 text-[0.8125rem] leading-(--text-sm--line-height) gap-1 rounded-md',
        lg: 'min-h-10 px-4 py-1.5 text-sm gap-1.5 rounded-md',
      },
    },
    defaultVariants: {
      size: 'md',
