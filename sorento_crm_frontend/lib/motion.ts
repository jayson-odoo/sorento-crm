 ...
 * accelerates `opacity` via WAAPI whenever nothing reads the value every frame
 * (`!onUpdate`) - `scale`/`x`/`y` are their own motion values and are not in
 * `acceleratedValues` there, so they are unaffected by this constant. WAAPI's
 ...
 * inline style leaves the element on whatever inline opacity was in place
 * BEFORE this phase started - `0`, the pre-entrance value, on enter, and `1`,
 * the pre-exit fully-open value, on exit - which is exactly what a
 * real-browser frame trace showed: content flashed to `opacity: 1` moments
 * before its exit reached 0, and the overlay flashed to `opacity: 1` (darker
 * than the surface was at any point while open) on the frame before
 * AnimatePresence unmounted it. It never reproduces in jsdom, which has no
 * WAAPI and always takes the JS path this constant forces everywhere.
 *
