...
import { scopeTotal, type MockScope } from './documentMocks';
 ...
 * nothing more until the reader asks for figures, and the figures are the same ones the table
 * footer sums (AC-D1), never a second arithmetic.
 *
          ...
          <TabsList className="flex-wrap">
            {scopes.map((scope) => (
              <TabsTrigger key={scope.id} value={scope.id}>
                <span className="min-w-0 truncate" title={scope.label}>
                  {scope.label}
                </span>
                {showAmounts && (
