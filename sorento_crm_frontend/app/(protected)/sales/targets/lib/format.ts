 ...
 */
import type { TargetBasis, TargetMetric, TargetProductScope } from '../types/salesTarget.types';
...
export const METRIC_LABEL: Record<TargetMetric, string> = { amount: 'Amount', quantity: 'Quantity' };
export const BASIS_LABEL: Record<TargetBasis, string> = { ordered: 'Ordered', delivered: 'Delivered' };
...
export function scopeSummary(scope: TargetProductScope | null, count: number): string {
  if (!scope || scope === 'all') return 'All products';
  const noun = scope === 'categories' ? 'categor' : 'product';
  const plural = scope === 'categories' ? (count === 1 ? 'y' : 'ies') : count === 1 ? '' : 's';
  return `${count} ${noun}${plural}`;
}

/** "Every 2 weeks", "Every month", or "Off". */
export function splitSummary(every: number | null, unit: string | null): string {
