...
describe('summariseContainerRequest', () => {
  it('decomposes the need: on hand first, then SPO, then the ask', () => {
    const rows = [
    ...
    // not column totals.
    expect(s.fromOnHand + s.fromSpo + s.toAsk).toBe(s.need);
  });
