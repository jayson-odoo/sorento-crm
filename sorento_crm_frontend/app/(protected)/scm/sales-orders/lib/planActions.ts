 ...
 *
 * It lives in the toolbar's own **Actions** dropdown, beside Refresh and Upload sales
 * orders, and NOT in the bulk strip. The strip only exists once rows are ticked, so an
 * action that lives there cannot be found by someone who has not already guessed it is
 * there - and when it refused (over the board's bound) it refused as a greyed-out button
 * whose reason was a hover away, which is indistinguishable from a dead click.
 *
 * In the dropdown it is always present, always counts what is selected, and states its
