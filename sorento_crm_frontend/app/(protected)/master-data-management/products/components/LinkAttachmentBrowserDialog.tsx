...
interface LinkAttachmentBrowserDialogProps {
  open: boolean;
...
export default function LinkAttachmentBrowserDialog({
  ...
  /** Map attachment id -> filename for selected items (so we can show names when switching folders) */
  ...
  const { data: existingAttachments } = useProductAttachmentsByProduct(productId || null);
  const linkedIds = useMemo(
    () => new Set(existingAttachments?.map((pa) => pa.attachment_id) ?? []),
    [existingAttachments]
  );
  const availableAttachments = useMemo(
    () => attachments.filter((a) => !linkedIds.has(a.id)),
    [attachments, linkedIds]
  );
  ...
  const handleConfirmLink = async () => {
    if (!productId || selectedIds.size === 0) {
    ...
    try {
      ...
      toast.success(`${ids.length} attachment(s) linked to product.`);
      ...
      <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col">
                ...
                {isLoadingAttachments ? (
                  ...
                  <div className="py-8 text-center text-sm text-muted-foreground">
                    {attachments.length === 0
                      ...
                      : 'All files here are already linked to this product.'}
                  </div>
                ) : (
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={selectedIds.size === 0 || isLinking || !productId}
          >
            {isLinking ? (
