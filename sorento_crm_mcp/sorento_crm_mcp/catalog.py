...
CATALOG: tuple[ToolSpec, ...] = (
    ...
    ToolSpec(
        ...
        (
            ...
            "  • Response has >1 entries → map the user's free-text phrasing to one of the returned "
            "`name` values (case-insensitive, accept partials like `dealer` → `Sorento Dealer` "
            "when unambiguous) and pass that name as `access_levels`. If ambiguous, ask the user "
            "to pick.\n"
    ...
    ToolSpec(
        ...
        (
            ...
            "When the verb is ambiguous (bare 'orders in February', 'orders last week', complaint Delivery-Order discovery), "
            "DEFAULT to `order_date_from` / `order_date_to`. "
            ...
            "  'orders in February' (no verb)        => order_date_from / _to = Feb 2026\n"
