...
CATALOG: tuple[ToolSpec, ...] = (
    ...
    ToolSpec(
        ...
        (
            ...
            "the live ones, plus at most the latest ended one per type where that type is still honoured. "
            "Superseded and no-longer-honoured promotions are NOT retrievable here: `active` is ignored, and "
            "date filters narrow the served set rather than opening up the archive. If the user wants a "
            "full history (\"every promo that ended in July\"), say that list is not available from here "
            "rather than presenting a partial one as complete.\n"
            "period_from / period_to (YYYY-MM-DD) scope the served set by date; `date_mode` picks which "
            "promotion date the window tests:\n"
            "  • `overlap` (default) — promo running during window ('valid/running during X')\n"
    ...
    ToolSpec(
        ...
        (
            ...
            "Sorento currently serves — live promotions, plus at most the latest ended one per type where "
            "that type is still honoured. Lines under a superseded or no-longer-honoured promotion are NOT "
            "retrievable here and `active` is ignored, so never present a result as the full history of a "
    ...
    ToolSpec(
        ...
        (
            ...
            "serves — live promotions, plus at most the latest ended one per type where that type is still "
            "honoured. Documents of a superseded or no-longer-honoured promotion are NOT retrievable here "
            "and `active` is ignored, so never offer a result as the full set of past promotion flyers.\n\n"
            "COMPANY SCOPE: optionally pass `contact_id` (Respond.io contact id) + `space_id` to scope "
            "results to that contact's company/companies; omit both for all-company results."
        ),
