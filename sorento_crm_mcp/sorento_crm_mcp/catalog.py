...
CATALOG: tuple[ToolSpec, ...] = (
    ...
    ToolSpec(
        ...
        (
            ...
            "SIMPLEST CALL, and the one to reach for when answering a contact: pass `contact_id` "
            "(+ `space_id`) and NOTHING ELSE. That alone is a valid narrowing filter, and it returns "
            "the handful of documents that contact may have - the dealer baseline plus their own "
            "grants. You do NOT need to resolve a document type first. Read the returned file names "
            "and pick.\n"
            "FILTER BY UUID (optional): `attachment_ids` (canonical attachment UUIDs csv / JSON / "
            "repeated), `directory_id`, `attachment_type_id` (SINGULAR - there is no "
            "`attachment_type_ids`), `uploaded_by` (all canonical UUIDs). "
