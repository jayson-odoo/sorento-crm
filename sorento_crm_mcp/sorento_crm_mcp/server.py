...
# Query params whose value is a JSON OBJECT, not a scalar. Generated as
# `dict[str, int] | str` instead of the `_scalar_union` below, because FastMCP's
# `pre_parse_json` (mcp/server/fastmcp/utilities/func_metadata.py) json.loads ANY
# string argument whose declared annotation is not exactly `str` - so the compact
# JSON string the backend route expects is turned into a native dict BEFORE
# Pydantic validation, and a scalar-only union then rejects it ("Input should be
# a valid string ... input_type=dict"). Both shapes therefore have to validate;
# `_normalize_query_value` re-serializes a dict to the same compact JSON string
# on the way out, so the backend keeps receiving its `Optional[str]` either way.
TOOL_OBJECT_QUERY_PARAMS: dict[str, tuple[str, ...]] = {
    # Dealer stock verdict (D20): the per-product {product_id: quantity} map,
    # parsed backend-side by `parse_requested_quantities`.
...
def _normalize_query_value(value: Any) -> Any:
    ...
    compact, key-sorted JSON string the backend route parses.
    """
    if value is None:
        return None
...
def _compile_tool(spec: ToolSpec):
    ...
    # Params declared as JSON objects take an object-first union instead, so the
    # value survives FastMCP's `pre_parse_json` (see TOOL_OBJECT_QUERY_PARAMS).
...
def create_mcp_app(settings: Settings) -> FastMCP:
    ...
    register_ideation_tools(mcp, settings)
    logger.debug("Registered ideation tools")
