...
* `**[system-management/](./system-management/)**` - admin/ops reference: operational logs and configuration tables (imports, integration logs, scheduled tasks, email/WhatsApp delivery, import column mappings), troubleshooting a failed notification, reading/retrying a chatbot turn trace, what the chatbot remembers between messages (its focus and its one open question) and how to watch a parser shadow window before promoting it, and the AutoCount integration contract.
...
* ✅ System Management - drafted (operational data reference, troubleshoot failed notifications, chatbot turn trace + retry, chatbot focus / one open question memory model, chatbot parser shadow window, AutoCount integration contract, import column mappings).
