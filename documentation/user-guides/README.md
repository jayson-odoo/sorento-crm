...
* `**[system-management/](./system-management/)**` - admin/ops reference: operational logs and configuration tables (imports, integration logs, scheduled tasks, email/WhatsApp delivery, import column mappings), troubleshooting a failed notification, reading/retrying a chatbot turn trace, the chatbot's "which products have X" set answers and paging (plus the Lookup Sets an admin fills in to teach it new words), and the AutoCount integration contract.
...
* ✅ System Management - drafted (operational data reference, troubleshoot failed notifications, chatbot turn trace + retry, chatbot "which products have X" set answers, AutoCount integration contract, import column mappings).
