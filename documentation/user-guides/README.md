...
* `**[system-management/](./system-management/)**` - admin/ops reference: operational logs and configuration tables (imports, integration logs, scheduled tasks, email/WhatsApp delivery, import column mappings), troubleshooting a failed notification, reading/retrying a chatbot turn trace, the chatbot's "last purchase cost" answer and the per-contact Field reveal that gates it, what the chatbot remembers between messages (its focus and its one open question), how an escalation to a team is routed (which team, whose brand, and the "did you mean" step for an unknown product code), and how to watch a parser shadow window before promoting it, and the AutoCount integration contract.
...
* ✅ System Management - drafted (operational data reference, troubleshoot failed notifications, chatbot turn trace + retry, chatbot "last purchase cost" answer + Field reveal gate, chatbot focus / one open question memory model, chatbot escalation-to-a-team routing (which team, whose brand, did-you-mean for an unknown code), chatbot parser shadow window, AutoCount integration contract, import column mappings).
