...
A verify can come back **skipped**, reported in the result toast:
...
* **no longer in the list** - the code is gone from the catalogue.
...
3. The result toast breaks down what happened per code, e.g. *42 verified, 1 skipped - changed while you were reviewing*. Skipped rows stay selected so you can deal with them; acted rows are released and updated in place.
...
* **Verify** stamps the code as rendered on this page. If the values changed under you, the page reloads them and you verify again.
* **Unverify** asks for confirmation, same as the worklist.

Whatever you do here shows on the worklist when you go back, with no refresh needed.

If a value is wrong, correct it in the values table on the same tab. That is the whole fix; there is nothing else to acknowledge or dismiss.
