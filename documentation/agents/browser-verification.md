...
- An evidence run is the text log itself (steps, URLs, network calls, outcome) written into the plan - not a folder of screenshots. At most two screenshots per lane, each under 200 KB (the pre-push hook's `png-size` check enforces the cap); do not commit raw agent-browser snapshot dumps (accessibility-tree `.md` files).
