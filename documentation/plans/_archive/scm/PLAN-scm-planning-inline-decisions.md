...
| R11 REVERSED (captain's ruling, 8 Sep 2026) | Confirm posts saved lines only | silence is no longer agreement. An uncovered line nobody has SAVED a decision for (pill still reads "Suggested") is left alone by Confirm: not posted, not counted, stays on the board undecided. "Save all suggested" / "Save as suggested (N)" remain the bulk way to agree with the engine before pressing Confirm. The one-button shape and the "N to confirm · M rejected" counter survive; only the population Confirm posts changes |
...
| C4 Panel layout | left: Ordered / Delivered / Outstanding / Incoming by the delivery date (read-only strip). Middle: Reserve (per location qty inputs, own + group + pools, with each location's Available beside the input), Borrow (existing rows + Add a borrow), Buy (switch + reason, as today). Right: Decision summary (composition that will be confirmed), the "N short / N over" hint (R9), **Why this differs** textarea (required when `amendNeedsReason`), **checkbox "This might be a system problem, flag it for investigation"**, buttons **Save** (ONE button, two verdicts, per the captain's 28 Aug ruling: approved when the composition still equals the engine's suggestion, `matchesSuggestion` in `fulfilmentBoard.ts`, and it resets the inputs to the suggestion and clears the reason; amended otherwise) and **Reject** (verdict rejected, reason required). Confirmed rows open read-only with an **Amend** button that unlocks the same panel (a re-confirm is a new revision, the pill still says Confirmed) |
