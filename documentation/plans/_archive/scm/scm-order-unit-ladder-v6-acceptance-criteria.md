...
- AC-U14 A line named in `uncover_line_ids` stops holding at commit: `confirm` neither nets its hold from the free stock the payload is judged against nor counts it as another line holding at that location (R14). Releasing a covered line and reserving its stock for a sibling in one confirm succeeds.
