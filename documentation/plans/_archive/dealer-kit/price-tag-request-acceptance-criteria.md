...
- **AC-M.26** `[BE][T]` Price Tag Request reaches nobody on the day it deploys. The foundation migration grants dealer-type access types `stock_inquiry` only, and a following migration strips `price_tag_request` out of every `contact_access_types.portal_form_types` array, so a database that already ran the earlier version lands in the same state as a fresh one. Running the strip a second time changes nothing and leaves the other kinds in each array untouched; its downgrade does nothing at all, because a grant is an admin decision and not something a downgrade re-invents. With no access type carrying the kind, `resolve_visible_form_types` answers without it for every contact, the landing does not offer it, and the portal price-tag routes refuse it - all through the resolver that already shipped, with no new gating code.
- **AC-M.27** `[FE][BE][T]` An admin grants the form from the Contact Access Types screen. Each row shows its portal forms as chips, or a dash for none, and the dialog that already edits the row carries a "Portal forms" field: a `SearchableMultiSelect` over the five known kinds - Complaint, Stock Inquiry, Purchase Request, Sponsorship Form and Price Tag Request - labelled exactly as the portal labels them from one shared map. Opening the dialog shows the row's current kinds selected; saving writes them, and the list chips update. `portal_form_types` is carried by the read, create and update schemas and answered on the wire; an unknown kind is refused with 422 and the row is unchanged. Adding Price Tag Request to an access type makes the form appear in that contact's portal landing dropdown on the next load, and removing it makes it disappear; no other field on the access type changes behaviour.

- **AC-M.28** `[BE][T]` A document number survives a deleted draft. Creating two requests, deleting the first and creating a third answers a number no row has ever held, rather than failing on the unique index; two companies creating in the same month get different numbers; and a create that loses an insert race takes the next number instead of answering 500.
- **AC-M.29** `[BE][T]` The tag sheet print payload is company-scoped. A request whose page belongs to one company resolves only that company's data: a line pointing at another company's product is absent from `resolvedData` and that product's code appears nowhere in the response, while the page's own line resolves as it always did.
- **AC-M.30** `[BE][FE][T]` A claimed request names its claimer. Claiming writes `assigned_to_id`, leaves `created_by` alone, and the claim response, the detail read and the listing row all carry `assigned_to_id` and `assigned_to_name` on the wire; an unclaimed request answers null for both. The listing also carries the line count, the salesperson's name and the promotion's name, and the portal list carries the line count its cards print. The CRM queue holds no drafts, and a draft cannot be claimed.
