...
### Confirmed against shared-service PR #86 (24 Sep 2026)

S1 is implemented in `jayson-odoo/foundryx-shared-service` PR #86. Two facts from that PR's
body, folded in for the S2 to S4 lane (this session could not read the PR directly - GitHub
access here is scoped to `sorento-crm` only - so these are taken from the task brief that
quoted the PR body, not independently re-verified against the shared-service diff):

- **Wire casing differs by route.** The intake endpoint (`POST /ideation/intake/create-idea`)
  stays snake_case on the wire, as documented in the Request/Response tables above. The S5
  public status route (`GET /public/ideas/{status_token}`) is camelCase on the wire -
  `ideaNumber`, not `idea_number` - so a sorento-side consumer of that route (none exists in S2
  to S4; S5 is shared-service + its own frontend only) would need to read `ideaNumber`.
- **`cancelled` and `voted` map to existing internal statuses.** `cancel: true` closes the draft
  by transitioning it to shared-service's internal `rejected` status (surfaced to sorento as the
  contract's `cancelled`); `duplicate_choice: "vote"` closes the draft by transitioning it to
  shared-service's internal `duplicate` status (surfaced to sorento as the contract's `voted`).
  Sorento only ever sees `cancelled` / `voted` on the wire (per the Response fields table); the
