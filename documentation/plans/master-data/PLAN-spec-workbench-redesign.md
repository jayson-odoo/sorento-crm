...
**Status:** S1 (Group A, registry list page) done at `c21422438`. S2 (Group B record page + E.1/E.2 value labels, `value_labels` mocked per D9) done at `769c8d7d4` - record page (`[specKey]/page.tsx`) with the three tabs sharing one view/edit layout, `useSpecKeyRecord`'s one-PATCH save, Delete wired to `useDeferredAction` (needs a backend `spec_key.delete` record-action handler before it can actually commit - not registered here, out of scope for a Phase 1 FE slice), `readableValue`/`readableEntry` take value labels and every E.2-listed consumer reads them; `SpecKeyEditor.tsx`/`SpecKeyProducts.tsx` deleted. S4 (Group D + E.3) done at `3a9f5c132` - migration `454_spec_registry_value_labels`, renumbered to `455_spec_registry_value_labels` on the 2 Sep merge with `origin/main` (both `454_order_inquiry_born_ack` and this slice branched off `453_shared_brand_attach`; rechained onto `454_order_inquiry_born_ack` to keep a single head, no merge migration needed) (D.1, applied to the shared dev DB via a direct `ALTER TABLE`, not `alembic upgrade head`, since `alembic current` sat far behind the head), `value_labels` on `_serialise` and the PATCH (D.2-D.5, test-first: red 15/green 351 across `test_spec_registry_pr2_routes.py` + `test_record_actions_s6b.py`), `product_spec_registry.delete_registry_key` shared by the DELETE route and the new `spec_key.delete` record action (D.6), and the S2 mock swapped out of `useSpecKeyRecord`/`useSpecRegistryMutations` now the column is real. E.3's browser round trip is deferred to the tester. S3 (Search ranking in Settings) and S5 (Spec Verification unverify countdown) were already done earlier on this branch. PR #520's Phase 3 review fix round done `3aaa80485`..`ea1c8fd04` - BL-1 (`applicable-keys` now carries `value_labels`), BL-2/D12 (`is_active` switch on the record card, both modes), item 12 (the 453 single-head migration test tolerates 454 chaining onto it), items 3-11 (value-row heading reads the label, `max_value` in both modes, pager/gear stay mounted+disabled while editing per the UAC B.2 exception, Seen in products reads labels + `rowHref` + always-mounted facets, the exceptions pill and the code-filter dismiss are the shared `Badge`/`Button`, the two Add-specification dialogs share one type-option list, the explainer paragraph is gone, Search ranking Save is disabled on an empty/non-finite draft and toasts on save). Hands-on feedback round 1 (D13-D17) done `881dda060`..`55f1bc66b` - registry list toolbar parity with Products, select column + row menu + deferred bulk delete that skips seed rows, record card gone fully read-only with a new Header tab first in the tab order, Rules loses its per-tab Edit, Seen in products facets moved into toolbar filters (Class/Source filter client-side on the loaded page - no backend param yet). S6 (reviewer + DoD gate) is next. Follow-up `r1b` closed that gap: `GET /{spec_key}/products` now takes `class_label`/`source` (tallies computed before those two filters so the dropdowns stay complete), and `SeenInProductsTab` sends them server-side instead of filtering the loaded page - test-first, backend 46 passed, frontend 120 passed, verified live on Brand (10,691 rows).
