...
| G4 | **Batch volume.** `derive_all` covers 11,415 codes. Enqueuing one event plus one RQ job per product row inside the batch's own commit would run the whole catalogue's embedding fan-out inside the import's commit hook, the exact failure `INLINE_REDERIVE_LIMIT` exists to prevent for derivation. | product_spec_change_listener.py:38-42 |
...
- **D4 - a large batch goes to the worker, like derivation does.** Above
  `INLINE_ENQUEUE_LIMIT` (same value and same rationale as `INLINE_REDERIVE_LIMIT` = 50)
  the ids are handed to an RQ task on the `imports` queue that enqueues them in chunks;
  when the queue cannot be reached, they are enqueued inline rather than dropped. Closes
  G4. One person's edit stays inline and is indexed on the next worker pass; a catalogue
  import does not run 11,415 embedding enqueues inside its own commit hook.
...
| S2 | Volume split at `INLINE_ENQUEUE_LIMIT` + RQ task | 51 changed rows hand off to the queue rather than enqueuing inline; queue unreachable falls back to inline and still enqueues all of them; the task itself enqueues in chunks |
...
4. A batch above the limit is handed to the worker; when the queue is unreachable the
   ids are still enqueued rather than dropped.
