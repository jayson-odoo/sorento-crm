...
| D15 | One action set per entity, shown in two places: the list row's "..." menu and the record page's gear. Same items, same order, same permissions (Impersonate on a user is the first case: list-only today). The 79 icon-button action columns become a "..." menu. Primary (Edit/open) stays a button on the record page and is the row click on the list. |
...
- **S4-01** The 19 strips in the inventory render the Users style (line, icon + label); the 17 keepers carry `variant="default"`; no `grid grid-cols-N` TabsList remains under app/. [FE][T]
...
- **S4-03** The 26 lists with a detail route and no row click gain `rowHref`; lists whose record is edited in a lightbox (Contact Access Agents, Brands, UOM) open that lightbox on row click; log and sub-tables have no pointer cursor. [FE][E2E]
- **S4-04** At 375: Product Categories shows the full Name; Product Specifications' out-of-date banner wraps normally; Ticket numbers do not break mid-string; Product detail Pricing Summary values never touch; the 35 unwrapped raw tables scroll inside their own container; the login card is centred at 1280; the dashboard has a page title; task cards wrap instead of truncating the identifier; the two floating buttons on Ticket detail do not overlap. [FE][E2E]
