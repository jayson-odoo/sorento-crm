...
- **S9-01** ESLint warns on `jsx-a11y/click-events-have-key-events`, `no-static-element-interactions`, `control-has-associated-label`, and errors on `text-[Npx]` in className strings (demo layouts exempt). Also warns on `react/no-unstable-nested-components` (`allowAsProps: true`), added after the combined-preview review found the Users list search box losing focus on every keystroke - its toolbar was a component declared inside `UserList`'s render body, so a parent re-render (every keystroke, since the search box's state lived in the parent) gave React a new component type to remount each time. 36 pre-existing sites warn at the level this landed; none were fixed beyond that one (warn level, like the jsx-a11y trio - fix opportunistically). [FE][T]
- **S9-02** Every unlabelled icon button has an `aria-label`; `role="content"` is gone and a skip link reaches `#main`; `IssuedKeyDialog` no longer traps the keyboard; the `outline-none` sites without a ring have one. Reconciled against the audit's estimate: **132 of 356** icon buttons in the current tree needed a label - the other 224 were already covered (`sr-only` text, an existing `aria-label`, or the demo/legacy path exemptions the a11y guardrail inventory already carries), so the audit's 306 was counting the tree before those exemptions were subtracted out. **12 of the audit's 73** `outline-none` sites got a `focus-visible`/`focus-within` ring; the remaining 61 were an over-count - most are `components/ui` primitives (button, tabs, input, input-otp, datefield) that already carried a ring before S9, which the guardrail inventory's own `RING_FIXED` comment records so a later refactor cannot silently drop one of the 12 real fixes into that same "already had one" bucket. [FE][T]
