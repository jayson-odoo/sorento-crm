...
| 6. Nested grid - fulfilment board cell breakdown, BEFORE the fix | Supply Chain > Project Demand > Fulfilment Planning, cell SRTWHBWP / 27 Feb 2023 (IR group, 48 outstanding), Stock tab, "Site pool subtotal" row expanded | 1440x900 | fail (this is the defect) | FOUR nested `overflow-y:auto` boxes, all overflowing at once: `dialog-body` 652/712, `cell-stock-table` 448/832, `stock-documents-panel` 315/662, `data-grid-scroller` 580/1045. The documents grid's box is 580px tall inside a 315px window, so its `position: sticky` `thead` sat at y=392.6 while the panel's visible top was y=425.6 - 33px above the clip, invisible. The outer stock table's `th` was `sticky top-0` and overlapped the "Site pool subtotal" row | `nested-grid-before.png` |
| 6. Nested grid - same cell, AFTER | same | 1440x900 | pass | ONE scrollable region: `dialog-body` 652/1907. `cell-stock-table` 1644/1644 and `data-grid-scroller` 1045/1045 have nothing to scroll, `stock-documents-panel` has `overflow-y: visible`. The inner grid's `thead` is `position: static` with an empty class list, the outer table's `th` is `static`, and the documents header ("Type / Document / Customer / supplier / Agent / Delivery / expected / Bin / Quantity") is fully visible with the subtotal row above it | `nested-grid-after.png` |
| 6. Nested grid - SCM reorder plan, On hand dialog | Supply Chain > Planning > Reorder Planning > run 04/09/2026 19:24 > row 1 SRTWC8503-RL > "On hand in the site pool" > BRW row expanded | 1440x900 | pass | The other real grid-in-grid site (`PlanRowDialog` > `OnHandTable` > `StockDocumentsPanel`). One scrollable region: `dialog-body` 652/1221. Inner scroller `min-w-0 overflow-x-auto overscroll-x-contain` with no `max-h-`, `thead` `position: static`, header fully inside the dialog body | `nested-grid-scm-onhand-after.png` |
| 6. Nested grid - promotions list, attachment detail modal | Marketing > Promotions > All Promotions, row "1. Bathroom Furniture.pdf" > "View details of ..." > Linkages > Promotions (1) | 1440x900 | pass | `AttachmentDetailModal` renders INSIDE the promotions `<DataGrid>` JSX, so its linkages grid is nested by React context. One scrollable ancestor (`flex-1 min-h-0 overflow-y-auto px-6 pb-6`, the modal's own body); the grid's scroller is 100/100 with no `max-h-` and its `thead` is `static` | `nested-grid-promotions-modal-after.png` |
| 6. Nested grid - complaint fulfilment items popover | Complaints > Complaints > a complaint > Fulfilment DOs tab | 1440x900 | not exercised | The fourth nesting site (`ComplaintFulfilmentOrdersSection`'s items popover, a `PanelDataGrid` opened from a cell of the section's own grid). No complaint in this dataset has a linked fulfilment delivery order - every one tried showed "No replacement delivery order linked yet", so the popover could not be opened. Its behaviour is unchanged by design: it names `scrollerMaxHeight="16rem"`, and a caller's own value beats the nested default, so it keeps both its window and its sticky header. Covered by `data-grid.nested.test.tsx` ("a nested grid that names only its own bound keeps a sticky header with it") | - |
