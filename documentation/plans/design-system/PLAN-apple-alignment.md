...
**Status:** IN PROGRESS - plan approved 2026-08-29 (lavish review + grill; D15, D16 added); S1 started 2026-08-29. S5 built 2026-08-30 (`feat/apple-S5-wayfinding`): PageHeader + the sweep of every module, GRN/SPO eyebrows, verb + noun labels, UUID titles, DataGrid `emptyAction`. S6 Phase 1 built 2026-08-30 (`feat/apple-S6-deferred-actions`): the deferred-action service, hook, countdown, toast and row dimming, with Users, Products and Orders on them against an in-memory `/pending-actions` (`services/pendingActionMock.ts`, one flag). Phase 2 owes the routes, the handlers, the two System Settings fields and the tests.
...
- Rollout: S6 = engine + primitives + Users, Products, Orders (record + list). S6b = the remaining ~80 `ConfirmDeleteDialog` importers and destructive `AlertDialog`s, one module per commit, then delete `ConfirmDeleteDialog`.
