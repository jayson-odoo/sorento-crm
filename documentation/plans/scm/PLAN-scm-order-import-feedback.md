...
**Status:** S1 to S4 merged to main (PR #143, which absorbed #147). S5 built (branch
`feat/scm-po-spo-history`) - developed stacked on S3 + S4, then rebased onto main so it
stands alone; backend only, pytest green, no browser pass needed - the channel has no new
screen. S6 built (branch `feat/scm-agent-master-ui`, PR #154, rebased onto main). Contract:
...
could not be followed, or a fact the file itself corrected).

10. **The SPO half lands in `purchase_orders` / `purchase_order_lines`, discriminated by
...
11. **Routing by prefix is applied to BOTH file shapes, not only the structured one.** The
...
12. **Nine rows disagree with their `Shipping Order` flag, not ten.** Re-measured on the real
...
13. **`Agent` on this book is NOT a salesperson, so it is not fed to the agent master.** Its
    ...
    resolves and stops.
14. **No cost and no currency are read from the structured export.** It carries no unit price
...
15. **Two facts the structured export states that the banded report cannot, and which are now
...
16. **Line identity is positional.** The structured export has no line-number column, so the
...
17. **The blank-Doc-No row carries `missing_doc_no`, and caption rows carry `not_a_line`.**
...
18. **BLOCKER, fixed: the outstanding-PO importer could turn history into supply.**
...
19. **DECISION, accepted not guarded: a history upload rewrites the header of a document
...
20. **The `source_system` split has no READER yet.** The PO list collapses both stamps to
...
`po_spo_history` (replayed by `bootstrap_env`). See amendments 10-17.
