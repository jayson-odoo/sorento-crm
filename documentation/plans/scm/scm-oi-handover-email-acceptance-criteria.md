...
| AC-H6 | [BE] | DEFERRED to slice 2 (16 Sep review S3): no writer today raises a CHANGE SO NO row that points at another sales order (the delta's CHANGE_REPOINT is a phase move inside one SO). Kept as the contract for the `was` pass-through: when a seam supplies `was.so_number` / `was.customer` / `was.project`, the drain carries them through unchanged. The template prints them once a producer exists. |
...
| AC-H20 | [BE] | Given a row carried by the cancel-and-re-raise site with the same qty and delivery date it already had, then no line is printed for it (nothing changed for purchasing). A carry whose qty or date changed prints once as a settled line with `was.*` set, never as a fresh ORDER. |
| AC-H21 | [BE] | Given a confirm that runs INSIDE a savepoint (`db.begin_nested()`, the planning-change apply and the outstanding-book upload path), when the savepoint is released and the root transaction commits, then exactly one dispatch happens and nothing is left pending on the session. |
| AC-H22 | [BE] | Given a carried row whose live handshake is missing (rejected, or never acknowledged), then the AC-H20 rule still applies: an unchanged carry prints nothing, a changed carry prints once as a settled line. |
...
| id | tag | Given / When / Then |
| --- | --- | --- |
| AC-H17 | [BE] | The dispatched context is `{ "handover": { "subject_scope", "verbs", "headline", "orders": [{so_number, customer, project}], "lines": [{so_date, so_number, customer, project, item_code, qty, delivery_date, remark, was}], "line_count", "link" }, "actor": {name, email}, "today" }`. `verbs` is distinct, in the fixed order ORDER, RESERVE & ORDER, ORDER BACK, PRE-ORDERED, ALREADY INBOUND, ADVANCE, DELAY, CHANGE SO NO, CANCEL BALANCE; `headline` = verbs joined by `, ` (RELEASE dropped from the vocabulary, review round 1 nit - no seam today ever produces an `IV_RELEASE` row for the recorder to see; add it back beside its producer if one ever exists). Dates are `dd/mm/yyyy`, quantities print without trailing decimals (`_qty_str`). `link` = `build_order_inquiry_link` of the first SO. |
