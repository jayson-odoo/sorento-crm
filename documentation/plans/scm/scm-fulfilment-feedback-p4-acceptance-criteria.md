...
- **AC-B3** On hand dialog = reorder planning's On hand lightbox: Location, On hand, Reserved, Free, SO qty, SPO qty, Available, PO qty, › documents, "Stock as of"; site pools only; total = cell.
- **AC-B4** SPO dialog (tabs Open to pools · History; SPO, Packing list, To, Qty, Received, ETA, Status) reads SPO-kind `purchase_order_lines` (R8a) and its open total equals the cell, which reads the same rows (pytest: the From SPO card and the cell equal the SPO-line sum, not `net_position.on_order`); Incoming PL dialog (Packing list, Container, Supplier, Qty, ETA, Status) reads unreceived `inbound_shipment_lines` and sums to the cell; PO dialog (tabs Open · History; PO, Supplier, Qty, Still to come, Unit price, Issued, ETA, Status) reads `purchase_order_lines` and its open "still to come" sums to the cell.
- **AC-B5** `GET /api/v1/scm/container-requests/drill?supplier_id&product_id&kind` returns `{kind, rows, total}` and `total` equals the corresponding `_stock_context` figure for that product (pytest asserts equality for each kind on a seeded chain, incl. a set row drilling on its driver member).
...
- **AC-B7** The shared `PlanRowDialog` lives in `app/(protected)/scm/components/`; no `PopoverPortal` remains in `scm/loading-plan/components/`.
...
- **AC-D4** The PDF renders the same 10+1 columns with the same merges (rowSpan), fills and fonts, A4 landscape, one page break per ~35 rows, the title row first.
...
- **AC-D6** Without a retained file the document has the same 10+1 header, no merges, 商标 = company letter, 规格 = product size when known, 品名 = product name; the five-column fallback no longer exists (pytest asserts the header of the no-file case).
