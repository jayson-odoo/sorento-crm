...
- **Incoming (SPO) is no longer a rung.** SPO stays INSIDE the group net (AutoCount's available already counts it); the rung and its proof row go. A LATE SPO reaches a line through Link SPO on its order-inquiry row.
- **The water, ruled 27 Aug (review round).** The NET stays date-blind - it is the group's position, not this line's promise. What a LINE may DRAW off that water at question 1 is bounded to the SPO quantity arriving ON OR BEFORE its required date (`fact.timely_refs` / `_group_water` carry the dates). Late water is NAMED in question 1's own sentence with its date ("30 on the water to MWH-IB arriving 1 Mar 2027, after the required date, so none of that is counted here") and never drawn. Water drawn at question 1 is an `incoming` (`timely_spo`) component under question 1's own rung, NOT a `reserve`: no `so_line_allocations` hold is written against goods that are not on a floor. Floor stock drawn at question 1 stays a `reserve`. One question, two component kinds, and both halves come off ONE ledger - `fact.timely_qty` IS the water share the question offered, so a `ConfirmLine` posting more `timely_spo_qty` than that is refused with the reason, and the Reserve recheck is seeded from the floor candidates alone. The water is distributed per location (`fact.group_net_by_location`) in the floor draw order, and the sentence names what was DRAWN rather than what was offered, so question 1 reads "Yes, 40 - it was drawn as 20 from BRW-IB, 20 on the water to MWH-IB". A placed PO offsets the water FIRST and the pool redirect second (`_apply_placed_offset`): a PO replaces expected supply before it displaces stock on a floor, and the trail step it annotates is `own` on a v5 proposal (there is no `incoming` step any more), matched on the kind when the step's own location and the cut's disagree - which they do whenever the water came to a sibling.
- **A CONFIRMED water line reads the same as the suggestion beside it.** The decision records the SPO share PER LOCATION with question 1's own rung (`BoardLineDecision.incoming[]`, beside the `timely_spo_qty` total `ConfirmLine` takes it back in), so both sides of the decision strip file it under **Use own location**, the sixth card does not come back at confirm, and the cell table's Taken keeps counting it. Without that split the confirmation named the line's own location whatever sibling the goods were coming to and carried no rung at all.
- **An answered refusal says nothing.** A cell whose only order-inquiry rows are refusals CS has already answered keeps the inquiry NUMBER it was last told about (the latest by `rejected_at`) and reports `ack_state: null`, so the grid stops printing "Rejected by purchasing: no reason given" over a line somebody has re-decided.
- **The strip's sixth card.** The separate Incoming input goes for a v5 line - the engine's water share is the only way `timely_spo` appears, and it totals inside **Use own location** with the rest of question 1's answer. The **Incoming supply** card is HIDDEN when it reads 0 and 0 (it counts only decisions frozen under an older ladder) and the grid narrows from six columns to five; the other five cards stand for questions and keep their place at zero.
...
- **Staleness:** a line with NO decision shows the LIVE suggestion; the frozen `proposed_components` snapshot is read only beside a decided line (that is what Suggested-vs-Decided is for) and is re-written at every confirm. The "(before ladder v5)" tag then appears only on a decided line whose snapshot predates v5.
...
| AC-B12, the balance invariant | `timely_spo 10 + pool 60` | `reserve 10 (group) + pool 60` | same 70, one rung different - and the REASON TEXT changed with it, from "Timely SPO 10: SPO ... arrives on ..." to "Reserve 10: BRW-BB gives 10 of the 10 the BB group can cover this line with" (`BALANCE_INVARIANT_STATED` pins both sentences verbatim) |
...
**Two consequences worth naming.** `group_net` has no arrival-date term, so a group whose only cover is a LATE SPO nets positive on it - and, since the 27 Aug water ruling, still does not DRAW it: the net is the group's position and the draw is this line's promise. `tests/scm/test_project_supply_service_ladder.py::test_a_late_spo_is_inside_the_groups_net_and_outside_what_the_line_may_draw` pins both halves in one fixture. And the cell table's batched stock reads now cover every active warehouse, because which group is the donor is not known until the engine has composed - the board used to leave a cited donor's figures null on purpose, and a block of dashes is not a subtotal anybody can check. That read is one query (the supply service's own cached `_active_warehouses`), and the per-cell donor-group scan behind it is indexed once per board rather than re-run per cell (39 statements to 37 on the query-bound fixture).
...
- **The confirmation judges INCOMING cover by the dated walk too (captain, 8 Sep 2026).** The
  ...
  applied to the water. The walk composes `timely_spo` out of `use_candidates_for`'s own-half
  water candidates - the DATED reading, what the one assignment gave this unit by its own date
  (`_drawn_at_own_date`) - and the board shows that same walk through `compose_lines`. The
  recheck bounded the posted quantity at `fact.timely_qty`, which is the water inside
  ...
  AC-L10 is unaffected: where the dated walk offers nothing either, the line still buys.
  Two things deliberately NOT changed here, each its own decision to take: (a) the OTHER
  project groups' water is not in the seed, because step 1's offer half is bounded by R-M's
  lending-group budget (`_group_budget_key`) and this scalar has no ledger for it, so widening
  it would trade the refusal for a double-promise across two units; (b) `_snapshot`'s water
  SPLIT still walks the undated `_group_take_candidates` alone, so on an oversold group it
  falls through to its "no water on offer" row - named at `fact.own_code`, carrying no rung.
  Where the water lands at the line's own bin (SO419851's case) that row is right and only its
  rung is missing; a unit whose dated water sits at a SIBLING bin would be named wrongly.
  Pinned as-is by `tests/scm/test_confirm_timely_cover_dated.py`. UAC AC-L5b.
