...
**R9. The send is a dialog with a channel.** "Send to supplier" opens "Send this request" (replaces the bare AlertDialog): radio **Email** / **Chat**. Email: **To** = address chips, prefilled with `suppliers.email`, the user adds or removes addresses (validated, at least one), optional note line. Chat: a Respond.io contact picker (server-searched over `respond_contacts`, prefilled with the contact whose phone equals `suppliers.phone_number` when one exists), otherwise "No chat contact for this supplier yet" with the picker still usable. Confirm sends on the chosen channel ONLY: one notice row per send, `channel` = the choice, new column `recipients` JSONB (email addresses, or the respond contact id + name). The always-written skipped chat row goes. Migration `442_supplier_notice_recipients_opens` (**numbered 442, not 441**: S1 and S6 took 440 and 441 in this lane; it touches only `supplier_notices`, so it is order-independent among the three).

*Built 28 Aug, one deviation:* a send with ZERO addresses is refused (422 `no_recipients`) rather than written as a `skipped` notice. That supersedes AC-F3's "a supplier we cannot reach still gets a notice" for the container request only - the dialog just asked who to send to, so "nobody" is a mistake, not an outcome, and the document is still obtainable without sending through `POST /container-requests/document`. The loading-plan approval path (`approve_and_notify`) is untouched and still writes a row per channel with the skipped-when-unreachable rule.

**R10. Chat = WeChat, through the composer's send path, not a new one.** Captain (27 Aug): "chat should be using wechat" (suppliers are in China). The chat row is the supplier's WeChat contact on the Respond.io WeChat channel (Respond.io offers WeChat Official Account as a channel; whether the Sorento workspace has one connected is verified in S3 Phase 2 before the send path is wired, and stated in the PR if it does not). The picker lists Respond.io contacts on that channel, prefilled by the supplier's phone. The send calls the same outbound service the unified composer uses (text + link inside the window, the approved template outside it, outbox row always logged), with the xlsx as the media attachment where the channel allows and the link in the text either way. If no approved template exists or no WeChat channel is connected, the send is refused with that reason; connecting the channel / seeding the template is a Respond.io task (needs the captain's go, n8n rule).

*Built 28 Aug, three findings and one deviation:*
- **The Sorento workspace has NO WeChat channel.** `respond_channels` holds exactly one row, `whatsapp_business` (Internal, +60 11-1673 1179, synced 2026-07-08). So every chat send is refused today with `wechat_channel_missing` until one is connected. Detection is `respond_channels.source ILIKE '%wechat%'` on an active channel - the table the template sync already fills from `GET /v2/space/channel`, so no live call sits on the send path.
- **The composer's function is `respond_chat_template_service.send_chat_message_for`** (the one `sla_service._deliver_conversation_message` calls for both the ticket drawer and the Conversations inbox). It owns the window branch, the template, the Respond errors and the `integration_log` outbox row on success AND failure. Called, not copied; its signature is unchanged. Its out-of-window template comes from a new use case, `supplier_request_chat`, added to `TEMPLATE_DEFAULT_USE_CASES` / `CHAT_TEMPLATE_USE_CASES` so an admin maps an approved template on the existing screen. Nothing is seeded.
- **The refusal happens BEFORE anything is written** (no notice row, and above all no retired token), which is what AC-C5's "nothing else changes" asks for: retiring the supplier's live link for a send that then could not go out would leave them holding a dead URL and no replacement. A notice reads `failed` with `last_error` when the send was attempted and Respond.io refused it, which is the case AC-C4 names.
...
**R12. One sheet model, three renderers.** `supplier_document_model.py` builds `SheetModel{title, columns[10+1], rows[], totals, source}` once; `xlsx`, PDF and the public page render it. **Built 28 Aug, one correction to this ruling:** the merges are read PER CELL (`Cell.rowspan` / `Cell.covered`), not per row, because the reference file's merged ranges are NOT aligned across columns - `A40:A43` and `A44:A50` are two 序号 families inside the single volume merge `H40:H50` / `I40:I50`. A row-level `family_span` alone would have had to invent a family boundary the supplier did not write. `family_span` survives as the 序号 column's span (0 on a row that continues a family), which is what the appended-row numbering reads. Column 11 = `需装数量 / Qty to load`, styled as column J (yellow, 14pt, bordered), header bold; a 0 ask leaves the cell empty (AC-C3 stands). Requested rows not on their list are appended below the last family, own 序号 continuing the count, same styling, `备注` = "不在库存表 / Not on your list". The `合计：` row keeps their three sums and adds the sum of column K.

**R13. With a retained stock list the xlsx IS their file.** `_with_qty_to_load` loads the retained workbook with openpyxl and writes column K into it (header + cells + total), leaving every existing cell, merge, width, fill and font untouched; appended rows copy the style of the row above. Set lines print the supplier's own code (AC-F12.6). The PDF and the public page render the same model with the same fonts (宋体/Calibri stack), fills and merges; the public page keeps its bilingual header labels as a second header line under theirs. Without a retained file (PI stand-in, no file): the model is built from our data in the same 10+1 columns, no merges (we hold no family information), 商标 = the product's company letter (S / C / M, from `companies.code`, blank on a set line, which carries no product id), 规格 = the supplier snapshot's own `spec` (there is no size column on `products`, and their spec is the only size we hold), 品名 = product name; the old five-column fallback is deleted. An `.xls` (OLE2) retained file cannot be opened for writing by openpyxl at all, so it is read for its VALUES and rendered fresh in the same eleven columns: their headers and row order survive, their cell styling never reached us. A golden test opens `库存明细.xlsx` (committed fixture), runs the builder, and asserts merges, fills, fonts, widths, the title row and column K.
...
|---|---|---|---|
| S1 | R1-R6: migration 440, plan row + list + Plan-a-container dialog with the dropzone, detail toolbar, Save/edits, cancel/delete | 4.5 | A |
...
~22 dev-days. Order A: S1 -> S2 -> S3. Order B: S6 -> S4 -> S5 -> S7 (S6 first: two of its items are verified defects). Migration chain: 440 -> 441 -> 442 on top of main's head at branch time (438 today; 439 lands with #354, re-check `alembic heads` before the first migration commit). Dev DB: apply DDL via `Operations.context`, never stamp (shared-DB rule).
