...
~22 dev-days. Order A: S1 -> S2 -> S3. Order B: S6 -> S4 -> S5 -> S7 (S6 first: two of its items are verified defects). Migration chain: S6's numbering seed took **440** (`440_pl_draft_numbering`, on `438_merge_price_supplier_sets`, committed first so the other lane can chain); S1 and S3 take 441 / 442 on top of it. Re-check `alembic heads` before each migration commit (439 lands with #354). Dev DB: apply DDL via `Operations.context`, never stamp (shared-DB rule).
