...
**One live link per PLAN, not per supplier (fix pass, supersedes part 3).** Part 3 retired every container-request token of the supplier on a send, and `cancel_record` did the same on a cancel. Two plans against one supplier is the ordinary case (a September container and an October one) and each holds its own current ask, so either action killed the link the supplier was working off for the other. `_retire_public_tokens(db, supplier_id, loading_plan_id=...)` now scopes to the plan when one is named; the send retires that plan's previous link only, cancel passes its own plan, and `GET /supplier-notices` takes an optional `loading_plan_id` so the record page (R3) lists its own sends rather than the supplier's.
