...
| AC-E03 | PASS | browser 4.1 (Project 480 / Retail 186 / Unclass. 12 stacked, drills per channel, one row per product) and 4.4 real-stack (AC15S row Project 2 / Retail 0 / Unclass. 2 off live `demand_class`); pytest `test_product_grain_summary` one-row-per-product + channel readings; legacy stored values kept - `test_order_summary_routes` legacy rows; vitest `SummaryOrderReportView` (33 tests: stacked readings, plan-grain chip, grain-lock note, legacy Unavailable cells) |
...
| AC-E06 | PASS* | pytest `test_product_grain_summary` / `test_summary_order_service`: MOQ + order multiple applied once to the product total, unclassified never enters `suggested_qty`; real-engine freeze tests pin shared facts counted once AND a covered location still stating its demand on the product row (`test_a_covered_location_still_states_its_demand_on_the_product_row`) |
...
| AC-F08 | PASS* | Was the defect: network-scope runs froze basis locations with no warehouse identity. Fixed (worknotes BE-6); browser 4.4 real-stack locations drill shows named BRW / BRW-BB rows, channel split, shared facts once, "Suggested once at the product: 37", "Chosen: 37", split back to locations; vitest `ProductLocationsPopover` (17 tests: loading / error / empty / data / legacy-Unavailable) |
| AC-F09 | PASS | pytest `test_plan_grain_policy` (stamped once at create from policy, never NULL, later policy edit does not move it; decision write in the other grain rejected; legacy runs accept no decision in either grain - HTTP guard tests in `test_order_summary_routes`, plus section G route-level guards on `decide_covered` / `confirm_decisions` / `reset_run_decisions` added after review); PO worklist grain-scoped (`test_product_grain_summary`) |
