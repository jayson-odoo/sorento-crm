...
| 4 | Order inquiries | Purchasing (Joey) | Auto-link; Link PO on an ORDER row; Link SPO on an ORDER BACK row to the cited document; Unlink one (confirm dialog); filter Linked = po / spo / none; search by Eling's name | Linked n of q with document and PO / SPO badge; tiers same location > same group > pool > sibling; PO issue date first; Raised by and Raised at | Supply Chain > Project Demand > Order Inquiries | Raised by built (#301); links with I |
| 5 | Reorder planning | Buyer (Josephine, Mr Loo) | Run a manual plan with no warehouse picked; read the Project column for SO381895's products; accept one buy, adjust one, skip one | Project demand = unlinked remainder of raised OI rows only (M310-CR-PJ reads 0 at BRW-BB); no Unclassified column; one history badge; no "Consider N more" line; Use PO offered on retail rows only | Supply Chain > Planning > Reorder Planning | P1 / P5 / P6 built (#311); P3 / P4 / P8 this week |
| 6 | Place PO | Buyer (Joey) | Confirm the draft PO from the plan; key it into AutoCount per the Allocated-to panel; re-upload the PO book | OI rows read Linked to the new PO; PO detail Allocated to (outstanding, allocated, free, placements with needed-at location and "location differs"); links survive the re-upload | Supply Chain > Purchasing > Purchase orders | P7 and G this week |
