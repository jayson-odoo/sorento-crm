...
| — | Grid shows `Reserved 3` / `Reserved 2` green pills with tick, `Amend reserve` + `History` icons, after commit | `AC-RS-83-84-85-lines-reserved.png` | PASS |
| — | One outbox row per commit, each naming only the rows THAT call touched, with "N line(s) still to reserve" (AC-RS-80/81) | DB check below | PASS |
