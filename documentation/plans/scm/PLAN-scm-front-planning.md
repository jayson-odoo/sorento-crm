...
5. CS opens one side sheet. Each line shows open quantity, timely location SPO coverage, proposed
   Reserve, proposed Borrow, proposed Buy, the required date, and evidence for every source. Every
   proposed component states its reason next to its quantity, so the proposal explains why and not
   only how much: for example "Reserve 10: free stock at BRW covers the need by the required date"
   and "Buy 60: remaining uncovered need".
...
9. The next SCM run reads the confirmed unplaced Buy once. The active **Plan grain** is an
   admin-level policy setting (rollout default **Product**), not a choice the buyer or planner
   makes per run; the run stamps the configured grain when it is created. **Planning mode: Auto /
   Manual** remains the buyer's own separate selector.
10. Product grain shows one row per product, with stacked Project, Retail, and unclassified
    ...
    the product total. The per-location view shows the same frozen location facts with the channel
    breakdown visible; under Product policy it is a read and drill view, not a decision surface.
    The configured decision grain is the only actionable grain.
...
| Purchase execution | Purchasing | Decision set from the run's configured grain, supplier, keying state, and PO linkage |
...
Accepted Project PO + schedule
  ...
  -> Order Inquiry
  -> frozen SCM run
  -> configured plan grain (admin policy)
...
Every proposed component is emitted with a short reason string derived from the rule that produced
its quantity, so the proposal explains itself rather than presenting raw evidence: Reserve names the
source location or the BRW pool and its cap, timely SPO coverage names the arrival date, Borrow
names the donor location or project, and Buy names the remaining uncovered need. The strings are
deterministic for the same snapshot, are never generated by an LLM, and are frozen with the line
...
### 5.1 Plan grain policy and planning mode

Plan grain and Planning mode are two independent settings, and neither replaces or overloads the
other:
...
Location grain remains a permanent grain: it is actionable whenever the section 5.1 policy setting
configures it, and otherwise stays available as a read and drill view. It reads the existing
...
Each new front-planning run has exactly one actionable `decision_grain`, stamped from the
configured policy when the run is created:
...
The buyer may inspect both views at any time. `decision_grain` is stamped once from the admin
policy setting when the run is created and never changes for that frozen run, so there is no
first-decision lock and no competing-grain race: a decision write in the other grain is simply
rejected. If the other grain must become actionable, an admin changes the policy setting and a new
current run is created from a new frozen input snapshot; the old run and all of its decisions
remain immutable and auditable.
PO worklists read only the run's configured grain.
This preserves two real Buy planning grains without allowing both to order the same requirement.
...
- Add `front_planning_contract_version` and `decision_grain` (`product` or `location`) to
  `scm.reorder_run`, plus one admin plan-grain policy field in system settings defaulting to
  `product`. Existing runs keep both run columns NULL. New runs set contract version `1` and stamp
  `decision_grain` from the policy setting at run creation, so it is never NULL on a new run and
  never changes afterwards. Decision services reject every write when the contract version is NULL,
  so legacy runs are read-only without a decision-grain backfill, and reject every write whose
  grain differs from the run's stamped `decision_grain`.
...
- Add the admin plan-grain policy setting with rollout default Product, stamp each new run with
  the configured grain, and retain the independent Planning mode selector.
...
- Pin mixed-channel same-location demand, Project Buy pass-through, decimal chosen-quantity
  allocation at UOM precision, unavailable legacy breakdowns, rejection of every legacy-run
  decision write, and rejection of a decision write in any grain other than the run's stamped
  grain.
- Keep both grains available, stamp one configured decision grain per run, and make PO worklists
  consume only that grain.
...
| Both plan grains create purchases | One configured decision grain stamped per run from the admin policy setting; PO worklists ignore the comparison grain |
...
- No new optimization service, rules engine, or second approval workflow is added; the only new
  setting is the single admin plan-grain policy field.
...
- No per-run Plan grain selector is offered to the buyer or planner; grain is admin policy.
...
or plan language where different. A row carrying a **Revised 2026-08-17** sentence was changed by
the captain in the PR #192 review; the original answer is kept ahead of the revision for history
and no longer binds where the two differ.
...
| Q4 | Buying a discontinued product for committed customer demand is allowed, with a warning and required reason. **Amended 2026-09-24 (`PLAN-fp-decide-and-one-reason-24sep.md`, D2, owner ruling on #1217):** allowed, with a warning; the reason is optional. The discontinued refusal at confirm is removed; the discontinued warning is kept. |
...
| Q8 | Product and Location remain selectable and actionable Plan grain modes over the same frozen product-location facts. Product has one row per product with Project, Retail, and unclassified demand as stacked analysis; Location shows the same breakdown by location. MOQ and rounding apply once to the Product total. One decision grain is locked per new run, separately from Planning mode: Auto / Manual; legacy runs are read-only. **Revised 2026-08-17:** Plan grain is not selected per run. An admin-level policy setting in system settings decides the active grain, with rollout default Product, and each new run stamps the configured grain at creation instead of locking it on the first decision. Under Product policy the per-location view remains available as a read and drill view and is not a decision surface. The one-row Product analysis, once-only MOQ and rounding, the separate Planning mode: Auto / Manual selector, and read-only legacy runs are unchanged. |
