...
| AC-RS-78d | [BE] | Line reserved 30 on request #1 and 0 on request #2 (pill `Reserved 30`): an amendment to 10 releases 20 from #1's link (one `unreserved` event of 20), net 10; an amendment to 45 with row remaining >= 15 raises #2's link from 0 to 15 (one `reserved` event of 15), net 45; an amendment to 60 with remaining 10 is 422; an amendment to 10 without a reason is 422 (below T = 30 held by #1 + 20 requested on #2 = 50). T = net held by every answered request row except the latest + the latest answered row's `qty_requested`, capped at the line qty: a line of 36 asked 36 / got 10, then asked the balance 26 / got 0, has T = 36 (not 62), so amending to 36 needs no reason and amending to 30 does. |
| AC-RS-85c | [FE] | The Amend form prefills the line's net (the pill figure) and, when the line has more than one answered request, reads `Requested T across N requests` with T as in AC-RS-78d (`Requested 50 across 2 requests` for 30/30 + 20/0; `Requested 36 across 2 requests` for 36/10 + 26/26 on a line of 36); the State filter offers `Not reserved`. |
| AC-RS-83c | [FE] | The reserve icons (tick / pencil, amend / history, staged chip + Undo) render INSIDE the State cell, to the right of the pill, not in a separate column; no column with id `reserve_actions` exists. Owner, 24 Sep: "this pen can put right next to state?" |
