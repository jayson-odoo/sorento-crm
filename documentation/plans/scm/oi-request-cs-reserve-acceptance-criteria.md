...
| AC-RS-78d | [BE] | Line reserved 30 on request #1 and 0 on request #2 (pill `Reserved 30`): an amendment to 10 releases 20 from #1's link (one `unreserved` event of 20), net 10; an amendment to 45 with row remaining >= 15 raises #2's link from 0 to 15 (one `reserved` event of 15), net 45; an amendment to 60 with remaining 10 is 422; an amendment to 10 without a reason is 422 (below T = 0 held by #1 after release... see plan: T = earlier net + latest requested). |
| AC-RS-85c | [FE] | The Amend form prefills the line's net (the pill figure) and, when the line has more than one answered request, reads `Requested 50 across 2 requests`; the State filter offers `Not reserved`. |
