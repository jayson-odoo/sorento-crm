...
`GET /api/v1/project-sales/stock-debt` gains three params, all optional and additive to `query`, `group`, `only_debt`, `page`, `limit`:

| param | type | meaning |
| --- | --- | --- |
| `cutoff` | `YYYY-MM-DD` | drop demand lines with `required_date > cutoff` (R2, A2, A3); axis ends at `month_key(cutoff)` |
| `supplier_id` | uuid | keep products whose last supplier (A1) is this one; `none` keeps products with no supplier at all |
...
- `_demand(..., cutoff)`: one extra `SalesOrderLine.required_date <= cutoff` clause, `OR required_date IS NULL`.
...
- `list()`: filter rows by supplier after assignment, then axis, then totals, then page.
...
Cell drill (`/cell`) takes `cutoff` and `book` so the drill foots with the cell that opened it.

### Frontend (Stock Debt screen only)

- Toolbar (R13): `Search` | `Filters` (count badge) | spacer | `Export` (primary). The refresh icon button and the "Only products in debt" switch leave the bar. Filters panel, top to bottom: Book (All / Project / Retail, default All), Ownership group (hidden while Book is Retail), Supplier (`SearchableSelect`, options from envelope `suppliers` plus "No supplier", clearable), Cutoff date (date picker, clearable), Only products in debt (switch, default on). Active filters render as the toolbar's existing summary chips, each with its own clear; "Only in debt" shows as a chip only when it is OFF (the default is not a filter the reader needs told about). Refresh happens on window focus through react-query as it does today; no button.
- Columns: `Total` after the last month column, before TBA. Every month column, TBA, No date, No location and Total declare `footer` reading from `totals`. Footer row labelled "Total" in the product column.
