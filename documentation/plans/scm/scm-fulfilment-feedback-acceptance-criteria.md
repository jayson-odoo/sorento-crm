...
## F3. Packing list reads like a record, and prints its container workbook (R25)

- AC-F3.1 [FE] The packing list detail carries a `Toolbar` (title = container number, else shipment number; subtitle = the factories on the lines and the shipment date) with prev/next, ONE primary "Download packing list", a gear holding Edit / Import Container Status workbook / Delete, and "Back to packing lists"; beneath it six routed `variant="line"` tabs in order: Details, Proforma invoices, Shipment lines, Documents, SPO planner, Timeline. An old `?tab=` link redirects to the matching route.
- AC-F3.2 [FE] The Proforma invoices tab renders the source-invoice table (PI number link, revision, lines here of total, qty here of total, amount, Open) with "Uploaded by {name}, converted on {date}" beneath it, and the empty state "Read from a packing list, not drafted from a proforma invoice."; the Timeline no longer carries the Origin card.
- AC-F3.3 [FE] The create page asks for no Shipment Number; [BE] `create_shipment` numbers a container that arrives without one, and the field is still editable on the Details tab.
- AC-F3.4 [BE] `GET /inbound-shipments/{id}/packing-list/export` returns sheet `RMB` in the FSCU layout: the twelve-line header block (LOADING / ETD / ETA / CONTAINER / SEAL NO / SO / CONSIGNEE / SHIPPER / CHINA AGENT / FACTORY / FREE DAYS / DELIVERY WAREHOUSE), the two-row column header, one block per factory with its MOCHA lines under "<FACTORY> (MOCHA)", the lines numbered 1..n across the whole container, and panes frozen below the header.
- AC-F3.5 [BE] Every derived cell is a FORMULA: `CTN QTY = QTY / PCS PER CTN` where a pack size was stated (else the stated carton count), `CBM / CTN = L*W*H/10^6`, `TOTAL CBM = CTN QTY * CBM / CTN`, `TOTAL NW = NW * CTN QTY`, `TOTAL GW = GW * CTN QTY`, `TOTAL RMB = RMB * QTY`; each block subtotals QTY / PCS-PER-CTN / CTN QTY / TOTAL CBM / TOTAL NW / TOTAL GW / TOTAL RMB and carries its own amount merged down its rows; the grand total sums the SUBTOTALS, never the line range. A line nobody measured states what it knows and derives nothing.
- AC-F3.6 [BE][FE] The line's material, pcs per carton, carton L / W / H and net / gross weight per carton come off the proforma invoice (`convert_to_draft_shipment` copies all seven, weights per carton) and are editable on the Shipment lines tab as a draft until Save; the container's seal no, shipper, forwarder order ref and its clearance cost, China freight cost and insurance rate are typed on the Details tab; the footer apportions clearance and China freight by each company's share of the volume and insurance by its share of the amount, and a container with no costs typed prints the split and no apportionment.
