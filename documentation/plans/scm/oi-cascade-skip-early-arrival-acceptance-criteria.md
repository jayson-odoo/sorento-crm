...
* AC-EA-13 Order inquiries worklist, a seeded Jan 2027 row whose only candidate is an early PO: Auto link all leaves it Not linked, the PO cell shows the muted dash, no pill. The toast is the existing one for a press that placed nothing (`Nothing new to link yet` from `linkOutcomeText`; the `N skipped` arithmetic belongs to the Link selected toast, which this rule also feeds). Control: the same PO promised inside the window links on the next press. VERIFIED 17 Sep on :3082, evidence in `evidence/oi-cascade-skip-early-arrival/` (AC-EA-13-not-linked.png, -375.png, -control-linked.png).
