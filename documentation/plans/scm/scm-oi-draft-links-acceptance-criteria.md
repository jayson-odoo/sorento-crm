...
| AC | Result | How |
| --- | --- | --- |
| AC-D1 | PASS | pytest `test_a_board_confirm_raises_a_to_confirm_row_that_already_holds_its_document`; browser could not exercise a FRESH board-confirm (SO381895 has nothing outstanding on the dev copy - see section 10); the confirmed-mark family (same code path, one ack_state later) is browser-verified on SO404352's real rows |
...
| AC-D3 | PASS | pytest `test_two_rows_are_never_drafted_onto_the_same_units`, `test_reject_on_a_placed_row_frees_the_pos_remaining_for_the_next_candidate` (new) |
| AC-D4 | PASS | pytest `test_the_plan_still_ignores_a_to_confirm_rows_remainder` |
| AC-D5 | PASS (pytest + vitest); not walkable fresh in the browser | pytest `test_confirm_stamps_the_row_and_moves_no_link`, `test_confirm_fills_a_remainder_the_draft_could_not_cover`; vitest `OrderInquiriesClient.test.tsx` "AC-D5: Confirm selected"; SO381895 has no To confirm row to press Confirm on (see section 10) and no other real row may be acted on |
