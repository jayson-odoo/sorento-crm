...
- C7 Editing Reserve BRW-AM from 9 to 5 shows the hint **4 short** and Save is disabled; setting BRW to 19 clears the hint, and Save enables once the reason is typed (a composition that differs from the suggestion always needs the reason, `amendNeedsReason` unchanged).
- C8 No line of the form "24 outstanding = 0 incoming + ..." appears.
- C9 **ONE Save, two verdicts** (captain, 28 Aug: "if the suggestion is same as decision then it is approved, if suggestion different from decision then it is amended, so I just click on 1 button"). Save on a composition that still equals the engine's suggestion turns the pill to Approved, clears the reason and resets the inputs to the suggestion; Save on any other composition turns it to Amended; Reject requires a reason and turns the pill to Rejected. Save is always enabled while the composition is the suggestion, and enabled only on a balanced composition carrying its reason otherwise.
