...
R1. **On a covered line the only draft the server accepts is an amendment.** `save_draft` refuses any other verdict with 409 `board_line_already_confirmed`, message "This line is already confirmed. Amend it to change the decision, or undo the confirmation." Covered = an ACTIVE `SOSupplyDecision` on the mirror order (`ProjectSalesOrder.so_id == core_line.sales_order_id`) whose `line_snapshots[].core_line_id` equals the core line. A superseded decision does not count. **A line named by a row of an OPEN planning-change batch (`planning_change_batches.applied_at IS NULL`) is exempt** (review rounds 1 and 2, B1 and S1-R2): the batch board shows every line of the open batch as uncovered, whatever the row's own state, so the planner can approve the batch's proposal, and that approval is a real change against the frozen decision. Keyed on the batch rather than the row's `applied_state` because a superseded or failed row is terminal and would exempt the line for ever; the client picks its surviving batch on the same `applied_at` test, so the two sides share one rule. The client stops uncovering lines of a batch that already carries `applied_at` (a deep link to an applied batch, S2-R2). Order-inquiry cover (`inquiry_decided`) is out of scope: no evidence, and the panel already shows no Save for a line with no composition.
