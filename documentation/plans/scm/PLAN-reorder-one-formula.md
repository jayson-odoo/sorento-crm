...
Status: built, PR open (review clean)
...
- **AC-3's other half, fixed in the review round:** the location-grain `reorder_point` basis DOES reach `_emit_cell`, and there S4's first cut left the buy sized against `retail_net` - which is `net` with project taken back OUT - after deleting the "+ project_need" addition. Neither half then counted the firm demand: measured on the `ZZTCHRM-DJ` fixture (retail 30 + class-less 7 + firm project 12, on hand 0, ROP 0) the row sized 37 where `level - net` is 49, so 12 confirmed units were bought as 0. `_compute_cell` now triggers and sizes against `net - project_supply_reduction` (project inside; only stock CS has already promised a project comes off, because those units are on hand and therefore in `net`), and `_emit_cell` applies the AC-F03 display split - firm project first, capped at what is being bought, retail the rest - exactly as `_emit_product` does at the other grain.
- **Still adding project on top, and deliberately not touched: `_emit_pool` and `_plan_network`.** Both compute `recommended = retail_recommended + pool_project_need` and force-trigger when firm project demand exists. Neither is named in S4's scope, both are off the default path (`buy_scope` defaults to `warehouse`, pool netting is opt-in per product, and the estate runs on `reorder_level` = product grain), and both are pinned by green tests that assert that arithmetic on purpose (`test_a_network_sized_buy_states_its_own_replenishment_not_the_sum_of_its_cells` expects 5 where the strict one formula would say 0). Bringing them onto the one formula is a separate ruling, not a silent extension of this one (issue #837).
- **Own-pool stock is a FACT, never a decision part (frontend, 11 Sep).** `S = min(on hand, need)` is read off the frozen line and shown read-only (the panel's BRW row, the pill/sheet's "Stock S" part); it is never persisted. `PlanDecision.stock` means CROSS-LOCATION borrowed stock only - `proposeCover` sources carrying real warehouse ids - which only a warehouse-grain row can have. `coverForLine` therefore offers a product-grain row NO cover: its `on_hand` is `_product_agg_cell`'s `_total("on_hand")` over the same cells `plan_basis.locations` lists. Measured on run d9790035 (11 Sep 14:09): of 226 cover offers made to product-grain buy rows, 148 were locations already inside that row's own on hand, and the other 78 were INACTIVE warehouses - which `_resolve_warehouse_ids` excludes from the run but `cover_service._POSITIONS_SQL` (no `w.is_active` predicate) still offers. That second group is a cover_service defect, not a reason to keep product-grain cover; it is named here as the trigger for a follow-up (issue #836).
