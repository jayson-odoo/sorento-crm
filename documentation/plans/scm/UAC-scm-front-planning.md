...
4. **J04 - Compose supply.** CS opens one side sheet showing dated location SPO supply, eligible
   Reserve, explicit Borrow evidence, Buy residual, warnings, the reason stated for every proposed
   component, and the CS reasons required on each line.
...
8. **J08 - Drill by location.** The buyer opens the per-location view to inspect or work the same
   frozen product-location facts with the demand-channel split visible; the run's one actionable
   grain is the grain configured by the admin plan-grain policy setting, so under Product policy
   this view reads and drills rather than decides.
...
| J04 | AC-B01 through AC-B14, AC-G02, AC-G04, AC-G06, AC-H01 through AC-H04 |
...
### AC-B14 [BE][FE][J04] Every proposed component states its reason

Given a proposed line composition, when the side sheet renders it, then each system-proposed
Reserve, Borrow, and Buy component is shown with a short reason stated beside its quantity, for
example "Reserve 10: free stock at BRW covers the need by the required date" and "Buy 60: remaining
uncovered need". The reason string is derived deterministically from the rule that produced the
quantity, is identical for the same frozen snapshot, is never generated by an LLM, and is frozen
with the line snapshot at confirmation. Quantities shown with raw evidence but no stated reason do
...
Given a frozen SCM run containing multiple locations and channels for one product, when its
configured **Plan grain** is **Product**, then the product has exactly one row. Its SO and
Suggested columns show stacked Project, Retail, and unclassified readings, and its expandable
ledgers preserve each channel's evidence without making channel part of row identity. On a new run
`project_demand` (open Project-class SO quantity) and `retail_outstanding` (stored
`dealer_outstanding`) are classified by persisted `demand_class`, a missing class counts as
unclassified, and `project_demand` is shown beside `project_buy_qty` (confirmed unplaced Buy) as
two measures of one owner; legacy runs keep their stored values.
...
### AC-F02 [E2E][J08] The per-location view is never retired

Given a frozen run with `decision_grain = location`, when the per-location view is opened, then the
same product-location facts show separate Project, Retail, and unclassified demand beside shared
stock, SPO, PO, reorder, decisions, overrides, net positions, and allocation evidence, and its
decisions and overrides are actionable. Given a run with `decision_grain = product`, when the same
view is opened, then it shows the identical facts as a read and drill view with no decision
control; Product grain has not retired or replaced the per-location view.
...
Given a new front-planning run, when it is created, then `decision_grain` is stamped once from the
admin plan-grain policy setting, is never NULL, is not decided by the first saved decision, and is
not changed by a later edit of the policy setting; a decision write in the other grain is rejected.
Product uses the single `order_summary_row` decision keyed by `(run_id, product_id)`, Location uses
the existing recommendation decisions and overrides, and PO worklists ignore the other grain.
Neither worklist nor response contracts add a channel key. Legacy runs accept no new decisions in
either grain.

### AC-F10 [E2E][J07][J08] A created run keeps its configured grain

Given decisions have been saved on a new run, when the admin plan-grain policy setting is changed,
then that run keeps its stamped `decision_grain`, all existing decisions and overrides remain
immutable, and only runs created after the change use the new grain; making the other grain
actionable for the work in hand therefore requires a new current run. Given a legacy run, when
either view is opened, then its history is read-only and its unavailable channel breakdown is not
