...
- AC-19b (R14/R15/R16, superseding the original wording) The Filters panel holds, in order: Book (All / Project / Retail, default All), Supplier (`SearchableMultiSelect`, options from `suppliers` plus "No supplier" - R15, multi), Due date (shared `DateRangePicker`, clearable - R14, replaces the single Cutoff date), Only products in debt (switch, default on). The Ownership group control is GONE (R16). The count badge counts Book when not All, supplier(s), the due date range, and "only in debt" when OFF.
