...
- **AC-M0.11 (decoupling)** GIVEN the position/consumption views WHEN inspected THEN they read only canonical `public` tables (stock/ledger/orders/PO/SO) — a test fails if they reference `inbound_shipments`/`spo_allocations`/AutoCount-shaped tables.
