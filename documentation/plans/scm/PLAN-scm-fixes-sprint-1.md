...
**Status:** GRILLED 24 August 2026 (Q1 = A, Q2 = project + order_inquiry origin, Q4 = add BL-027 as S5; Q3 pending the captain's go). S1-S5 built, reviewed, findings applied; F05 done (ten stale PRs closed 24 Aug). PR #276 open, merge deferred by the captain (merge = deploy).
...
- **AC-E04 (24 Aug, :3020):** scoped run `97192a2c` (MWH pool, product SRTBT005; stamped `company_id` by hand because a script-made run has none and run history is company-scoped). Sidebar Supply Chain > Reorder Planning > run history > that run > expand SRTBT005 > click the MWH-RSV member row: dialog "How this recommendation was reached", Trigger = `reorder_level: net -1 <= level 2; 3 available at MWH (netting off)`. `errors` empty. Note: at product grain the reason text is reachable only through that dialog; the location-grain grid shows it inline.
- **AC-N01:** `tests/scm` on the branch: 563 passed, 1 failed (`test_m0_cp4_golden_set[C-FH24]`, data-golden, fails identically on the primary checkout; pre-existing), `test_m3_run.py` and `test_pool_netting_parity.py` deselected (2 + 2 pre-existing failures, verified identical with the engine change reverted).
