...
- **B3** Remaining count is shown next to the action whenever `remaining` is finite ("2 of 3 revisions left").
...
- **P3** Revision attachments are listed **by filename only**, images embedded for the current form only (the same reasoning as H6a). The names are the record of what was attached at that version.
...
- **P4a** With the option on, the document reads **current version first, then each earlier version newest-first**, each starting a new page (PDF) or its own sheet (Excel), each headed by its label, submitter and reason. That order matches the purpose the user stated: what it is now, then how it got here.
