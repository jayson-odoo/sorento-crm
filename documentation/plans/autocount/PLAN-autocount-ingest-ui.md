...
> **Status:** IN PROGRESS — Slices 1-7 DONE + gated. Slice 8 (SO/PO reuse+ALTER) BE DONE + 19 pytest + live-smoked (AC-SMOKE-SO-1, AC-SMOKE-PO-1), FE surfacing/source-gate in progress. ALL backend work complete. Migrations chain cbf3a0044924 → 302..309_autocount_so_po_pricing (dev DB on a parallel head; re-chain before PR = open task). Pre-PR: automate integration-role grants + re-chain migrations.
