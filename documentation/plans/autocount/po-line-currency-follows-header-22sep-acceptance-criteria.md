...
- AC-PLC-6: Backfill script dry-run reports the count and writes nothing; `--apply` flips any line (whatever feed wrote it) whose header currency is set and differs; a line whose header carries no currency, or whose currency already matches its header, is untouched.
