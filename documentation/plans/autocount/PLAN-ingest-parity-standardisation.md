...
| D14 | Absent = untouched, null = cleared, on every master (pydantic `model_fields_set`); insert defaults follow manual create. Addendum 2026-09-06: a blank string on an optional field = cleared too (what a blank xlsx cell means; the ESB sends `""` for a mapped-but-empty field and never null). Required identity fields still refuse blank |
...
- Decided as D23 (captain, 2026-09-06). Original finding: the outstanding SO upload REFUSES the whole file when any
