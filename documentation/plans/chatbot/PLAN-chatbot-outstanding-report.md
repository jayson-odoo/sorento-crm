...
| R19 | **NEW, owner ruling, 13 Sep 2026, confirmed.** EVERY scope question prints the SAME four header lines the REPORT prints, in the same order and the same words, above `Outstanding for which document?`: `Product:` / `Customer:` / `Location:` / `Order date:`, with `all` on any axis nobody named - never an omitted line. This REPLACES R15's partial header (which printed only the filters that were set, and only on the re-ask) and closes the question R15 left open about the FIRST ask: one writer, `lanes/business/__init__.py::_outstanding_scope_filter_lines`, for the first ask, the R15 refinement re-ask, the out-of-range re-ask and the R16 ask resumed after a customer picker. Live trace: `outstanding dealer quantity for chin chun product SRTKT39SS in 2026` -> picker -> `1` -> `Product: SRTKT39SS / Order date: 01/01/2026 to 31/12/2026 / Outstanding for which document?`, with the customer they had just chosen nowhere on it - "i have chosen the customer already ... what about the customer, sometimes i might even have dates, location filters, they should be stated down in this message also." The `Customer:` line prints NAMES, never the resolved uuids: they are worked out once in `run_fetch` (the resolver's own name, else the roster label or token the customer saw, else the code - a picked customer's `canonical_code` is a DEBTOR CODE, which D7 says is never printed) and stored on the filter set as `customer_names` beside `customer_ids`. The SAME dedupe lands on the report's own header at the route (`outstanding_report_service._customer_echo`, AC-1163): DISTINCT names in FIRST-SEEN order, because one company keeps one `customers` row per ledger and a live FULLSHUN ask printed `FULLSHUN SANITARYWARE SDN BHD` five times among fourteen. Never alphabetical (that put an asterisked SHOWCASE ledger first), never grouped into a family with a count (the owner turned that down), and a leading `*` or an `[A/C III]` suffix is the ledger's own data and stays verbatim. The presenter is unchanged - it prints the line the route hands it. |
| R19b | **CORRECTION to R19, owner round 7, 13 Sep 2026.** R19 said the scope question's `Customer:` line prints names; it did not say WHERE the names come from, and the first implementation took them from the resolved entity (its display name, else the roster label or typed token, else the code). Live, that printed the PICKER's own label - `CHIN CHUN HARDWARE SDN BHD (MCH, SRT)`, a company-code suffix that is not part of any customer name - while the REPORT two turns later named the eight ledger rows from `outstanding_report_service._customer_echo`. One filter, one line, two sources. The rule: the scope question's line is built from the customer ROWS for the stored `customer_ids`, through that SAME `_customer_echo` (so AC-1163's distinct, first-seen rule comes with it), and is byte-equal to the report's own line for the same ids. A roster label is a display string for the PICK, never a customer name. `outstanding_filters.customer_names` and the entity-name resolution order R19 introduced are REMOVED rather than fixed: they were the second source. |
| R20 | **NEW, owner round 7, 13 Sep 2026.** An OUTSTANDING ask's ambiguous-customer picker prints NO delivery hint - no per-line `- has DO` / `- no DO`, no closing "None of these have a ... DO." - and does not run the probe behind it. `pickers.annotate_customer` measures `crm_order_management_orders_list` rows carrying an `actual_delivery_date`, i.e. DELIVERED DOs (the owner's own 6 Sep ruling, for delivery enquiries), which is the OPPOSITE population from this report's DO block (DOs not yet delivered). Live: the picker stamped "- no DO" on every line and "None of these have a matching DO.", then the report showed a DO with 5 outstanding - "it is still kinda strange for me though, to say no DO, then later when i get the summary, there is DO." The hint cannot be made true for this ask by rewording it, and nothing here needs measuring, so neither happens: the decision is taken where the probe is armed (`resolve_gate.py`'s `If-customer-picker` arm) on the ask's own `order_status`, and the annotator's UNPROBED arm renders the bare picker with its wording untouched. Every other ask keeps today's hint - the axis is the order status, never the domain. |
| R21 | **NEW, owner round 8, 13 Sep 2026.** A did-you-mean pick CONTINUES the question the picker interrupted, whether it picks ONE option or ALL of them - the same carry a single pick already gets. Live: `outstanding dealer quantity for chin chun product SRTKT39SS in 2026` hit a three-family customer picker, and `all` ran the PLAIN order list for all three with no dates and no status, while `1` on the same ask armed the scope question correctly. "why when i say all for customer picker it didn't work, but when i choose 1 it worked?" The cause is ORDER, not a missing carry: the parser's raw read of "all" is `entity_op: "clear"` (over a menu it means the opposite - pick everything), the entity-operation executor consumed that CLEAR before the "ALL on a numbered menu" arm relabelled the turn `reuse`, and R16's carry of `order_status` / `date_filter_*` / `requested_attributes` / `is_active` lives INSIDE that executor's reuse branch. So the fix is one predicate (`_select_all_over_a_menu`) read at BOTH points - the executor takes the reuse arm a single pick takes, and the expansion below reads the same rule rather than a second copy of it. NO new carry was added to the all-arm (owner, this sitting: "why is it so inconsistent ... is it our design problem"). The roster's own picked entities still win over the reuse arm's restored prior ones - that is the pick's whole point - and everything else carries. For an outstanding ask the scope question then arms for the UNION of every picked family's `customer_ids`, with the R19b header naming every ledger of every picked family and the `Order date:` of the original ask; `crm_order_management_orders_list` is never called, and a PLAIN ask's `all` still lists every family exactly as today. One more repair the chain needed: the answering turn's skip of resolve+gate covered a carried CUSTOMER subject only, so a scope answer after an all-pick (whose stored subject also carries a PRODUCT) died at the gate with "I need at least one filter" - it now covers either subject, which is what D10 already says in words (the carried code wins over any re-resolution). |
| R22 | **NEW, owner round 9, 13 Sep 2026.** THE WAY OUT of an open outstanding question. Live, with `Reply 1 for the delivery order list.` on screen, `hi`, `hi`, `no` and `stop` each printed the same offer back: "wud i can't reset now?" Two shapes close it, both through the same door a new ask already uses (pending dropped, `outstanding_filters` gone, nothing fetched): **(a)** a DECLINE - the parser's own `is_affirmative: false` on a turn that picked nothing, named nothing and refined nothing - which gets ONE SHORT LINE and nothing else - `Okay, noted.`, the registry's new `offer_declined` key (code + `515_chatbot_offer_decline`, seeded the way `476_chatbot_reply_copy` seeded its eight, so the owner can edit the wording without a deploy) - and arms nothing. The first cut used `clarify_menu`, the nearest line that already existed, and the live check showed why that is wrong: "I see you're trying to decline, Let me understand more. Are you asking about any of these? ..." re-opens a conversation the customer has just closed. The other decline line, `escalation_declined`, names an escalation nobody asked for; and **(b)** a SECOND consecutive unreadable turn. The AC-1143(c) re-print therefore happens AT MOST ONCE per offer: the turn that re-prints stamps `reprinted: true` on the pending marker itself, and the next unreadable reply closes the question instead of printing a third copy, then takes its own ordinary path (for `hi`, the low_signal greeting, which is the right reply once nothing is open). A boolean on the marker, never a TTL: R2 rules this offer STICKY across picks and casual turns, and a countdown would close it behind a customer who is still reading it. Picks and R15 refinements are unaffected by the count. |
| R23 | **NEW, owner ruling, 13 Sep 2026.** LATEST FIRST in both detail lists. The owner's own SO list printed 14/12/2022, 01/08/2024, 09/08/2024, 20/08/2024: "it need to be the recent one not the old one, since if it is a long list, I want to see the recent one". `so_rows[]` sorts by `order_date` DESCENDING and `do_rows[]` by `do_date` DESCENDING, ties by document number descending (the higher number is the later document), and a row with NO date sorts LAST - an undated row is the least recent thing there is. Sorted in the ROUTE service, per R10: the presenter prints the order it is given and never re-sorts, so no MCP change (its two detail goldens carry one row each and exercise no ordering at all). |
| R24 | **CORRECTION to R15, owner round 9b, 13 Sep 2026.** A turn the PARSER classifies as a business question of its own - `message_type: "business_query"` with a NON-NULL `domain_hint` - is a NEW ASK under an open outstanding question, whatever axes its entities sit on. R15's refinement test was axis-only, so `delivery status for hanlim` (a customer entity under a PRODUCT-subject offer, therefore a different axis) was read as a refinement of the old product's report: it re-ran SRTWT7443 with `Customer: all` and re-offered, and the hanlim named in that very message never reached any tool. "I kind of can't escape this loop." A refinement is the turn that narrows what is already on screen and says so in the parser's own terms - `casual` with no domain ("i want to see this month only"), or `business_query` with NO domain ("only BRW", the parser reading a filter rather than a question). The test lives INSIDE `_outstanding_keeps_subject`, so there is still exactly one definition of what a refinement is. |
...
**Follow-up, not this PR (2): the shape the round-8 miss came out of.** The owner's question this sitting was not about `all` - "why is it so inconsistent, sometimes it works sometimes it doesn't, is it our design problem" - and the answer is yes, and it is visible in this file's own rulings list. The head carries one arm per OPEN-QUESTION KIND (the did-you-mean picker, `team_clarify`, the tier menu, `member_offer`, `outstanding_scope`, `outstanding_detail`) times one per ANSWER SHAPE (a number, the word "all", words naming an option), and each cell holds its own copy of "what survives from the question this answer interrupted". R16 wrote that copy for a numbered pick; R21 is the same rule missing from the cell next to it. The misses look random to a customer because each cell is written when someone hits it, not when the rule is made. The structural fix is PR #863's single dialogue state - one focus plus one open question, with one place that says what an answer keeps - and the NAMED follow-up is: once #863 merges, move the outstanding scope and detail questions onto that model and delete their arms (`head/output_exchange.py::_apply_outstanding_pending` and the `outstanding_*` carries in `tail/compile_state.py`; the parser-context option labels from D17 stay, they are the input the model reads). Until then no NEW open-question kind is added to the head.
