...
| R22 | **NEW, owner round 9, 13 Sep 2026.** THE WAY OUT of an open outstanding question. Live, with `Reply 1 for the delivery order list.` on screen, `hi`, `hi`, `no` and `stop` each printed the same offer back: "wud i can't reset now?" Two shapes close it, both through the same door a new ask already uses (pending dropped, `outstanding_filters` gone, nothing fetched): **(a)** a DECLINE - the parser's own `is_affirmative: false` on a turn that picked nothing, named nothing and refined nothing - which gets ONE acknowledgement composed by the lane (the registry's existing `clarify_menu` line; the only other decline copy is `escalation_declined`, "Escalation declined.", which names an escalation nobody asked for) and arms nothing; and **(b)** a SECOND consecutive unreadable turn. The AC-1143(c) re-print therefore happens AT MOST ONCE per offer: the turn that re-prints stamps `reprinted: true` on the pending marker itself, and the next unreadable reply closes the question instead of printing a third copy, then takes its own ordinary path (for `hi`, the low_signal greeting, which is the right reply once nothing is open). A boolean on the marker, never a TTL: R2 rules this offer STICKY across picks and casual turns, and a countdown would close it behind a customer who is still reading it. Picks and R15 refinements are unaffected by the count. |
