...
Status: IN REVIEW, small fix track (backend only, under ~300 changed app lines, no migration, no auth/RBAC change, no new ingest surface). PR #1300, branch `fix/chatbot-escalation-brand-from-focus`.
...
1. `lanes/escalation.py::_apply_focus_brand`: the brand of the product the escalation is about, read off the product row through a new `product_brand` seam. The product is `item["focus_products"]`, THIS turn's applied focus: this turn's named product, else the product in focus. It outranks `carried_brand` and `stated_brand`, and never the roster arms (`picked_member`, `company_pick`, `prior_state*`, `multi_company_unpicked`).
