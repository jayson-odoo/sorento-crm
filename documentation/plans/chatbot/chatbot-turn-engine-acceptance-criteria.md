...
- AC-008 `[T]` Given a slice PR, when it opens, then `tests/chatbot/COVERAGE.md` (generated by
  ...
  node the slice ports and no empty cell in the worlds matrix (`branch_kind` x shape) for the
  lanes it touches; fresh captures come from the n8n repo's `scripts/capture-fixtures.py`
  recipe, never hand-written. The 1,535 / 11 starting corpus is a floor. (C2)
- AC-009 `[T]` Given the world corpus, when S1 opens, then it holds 100+ worlds including
