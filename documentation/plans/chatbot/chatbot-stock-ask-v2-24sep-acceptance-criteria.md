...
- **AC-SA338 [BE]** Semantics of the answer: numbers one per line are positions in order and fill those lines, the rest re-asked; the list pasted back with blanks fills the filled lines and answers now, blanks skipped and named not checked; "that's it" / "done" / "itu saja" answers what is filled; "N for all of them" gives every line of the list N, over an open or a just-answered check; cancel drops the question. An item that places on no line, or two on one line, leaves the object unapplied.
- **AC-SA339 [BE]** A bare number after a check answered for two or more products never fetches and never reopens a bigger list: it asks "Is N for all K products, or for one of them?" over the same numbered products. A one-product check keeps AC-SA330.
