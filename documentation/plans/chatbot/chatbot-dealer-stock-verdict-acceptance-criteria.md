...
| D29 | Family expansion and a quantity. An inventory entity that CARRIES a quantity fetches its exact code only (case-blind); the resolver's family siblings (rows whose code equals or starts with the typed code) are dropped from the fetch spec for that entity when an exact match exists, otherwise the expansion stays (a family prefix). An entity without a quantity keeps today's family expansion for everybody: "stock for CB313?" lists the family, the block answers all of them, the task holds all of them as slots. A staff contact typing a quantity gets the exact code too, the deliberate cost. (Live pass run 3, case H rows 5 to 8; review rounds 6 and 7.) | ruled (captain) |
| D30 | The availability block must ride through the MCP render envelope (`presenters._PASSTHROUGH_KEYS`): the engine opens the task from the block's `needs_quantity` entries (D25), and before review round 6 the presenter used the block for the dealer's sentence and dropped it, so no task ever survived a turn in production while every reply looked right. Found by reading `chatbot.turns.envelope` for the failing turns. | ruled (captain, review round 6) |
| D31 | In `availability` mode a named product id stands for its CODE: `list_stock` expands `product_ids` to every product row sharing those codes (named id first) before the policy query, so the merged entry (D27) is judged on every company's rows on the fill turn too, not only the one id the task carried. `compact` and `detailed` untouched. (Live pass run 4, case A turn 2; review round 8.) | ruled (captain) |
| D32 | An availability reply is the verdict lines alone: no cross-domain zero-set sentence, no ladder rung (incoming or purchase order listing), no escalate offer minted by the ladder. Gate on the presence of a non-empty `stock_availability` block at the ladder's entry (`answer_bridge.apply_crossdomain_hit` carries the block into the item; `crossdomain_zeroset` returns off). A detailed or compact reply still climbs the ladder. (Live pass runs 5 and 6; review rounds 9 and 10.) | ruled (captain) |
| D33 | A bare number with no task open, no entities and nothing on the products axis plans no inventory fetch (the empty-subject refusal fires on a single-domain turn flagged `bare_quantity_only`); a bare number with an open task or a carried product still answers it; "reorder report" style intents keep scoping themselves. The MCP presenter caps the noted / missing lists at 10 codes plus "and N others" whatever the engine sends. (Live pass run 5, case F turn 3; review round 9.) | ruled (captain) |
| D34 | A cancel plans nothing: a `topic_reset` turn with no entities and no quantity closes the task aimed at its domain and plans no fetch whatever `intent_hint` says. A reset aimed elsewhere with a domain word still plans that domain; a reset that names a product still fetches it. (Live pass run 7, case F turn 2; review round 11.) | ruled (captain) |
| D35 | An availability ask that names no product never receives the catalogue as a question. The backend block returns `stock_availability: []` with `stock_visibility.needs_product: true` when the caller passed no product; the presenter says `Which product do you need? Send the product code and the quantity.`; no task opens. `detailed` and `compact` keep their catalogue paging. (Review round 11.) | ruled (captain) |
