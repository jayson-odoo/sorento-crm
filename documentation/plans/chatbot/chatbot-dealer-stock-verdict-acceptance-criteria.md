...
| D29 | Family expansion and a quantity. An inventory entity that CARRIES a quantity fetches its exact code only (case-blind); the resolver's family siblings are dropped from the fetch spec for that entity when an exact match exists, otherwise the expansion stays (a family prefix). An entity without a quantity keeps today's family expansion for everybody: "stock for CB313?" lists the family, the block answers all of them, the task holds all of them as slots. A staff contact typing a quantity gets the exact code too, the deliberate cost. (Live pass run 3, case H rows 5 to 8; review rounds 6 and 7.) | ruled (captain) |
