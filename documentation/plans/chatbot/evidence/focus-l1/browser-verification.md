...
## Re-run 2026-09-13, session `focus-l1c` - one short recheck, commit dac7e946b (topic-reset fix)

Stack unchanged (:3086), coordinator reported it now serves commit `dac7e946b` (a topic-reset
fix). New session `focus-l1c`, closed cleanly at the end - no daemon issues this time (avoided
the date-picker entirely, matched the instruction not to fight it and not to use Chat History).

Console > Contact `Jayson` (confirmed `contact_respond_id: "437264483"` on every request) >
**Prompt version left at its default** (confirmed `"v13 · full"` in the selector, and
`prompt_version: 13` echoed back on every response in this block).

### Run 1 - default v13 (the production label)

1. `stock SRTWT2635` -> `"Stock details found for the requested products.\n\n1. *Product Code:*
   SRTWT2635\n*Warehouse:* BUKIT RAJA\n*System Location:* BRW-SMC\n*Quantity On Hand:* 3\n
   *Outstanding:* 3\n\n_Data last updated: 04/09/2026 17:30:59_"` - `focus.products` = SRTWT2635.
2. `another one` -> **same stock reply verbatim, re-run for SRTWT2635 again**: `"Stock details
   found for the requested products.\n\n1. *Product Code:* SRTWT2635\n...\n_Data last updated:
   04/09/2026 17:30:59_"`. `focus.products` unchanged (still SRTWT2635, `set_at_turn` still 2 -
   never touched), `focus.domains.set_at_turn` bumped to 3 but same value. No topic-reset
   acknowledgement text at all this time (a different failure shape than the previous session's
   v13 attempts, which at least sometimes returned an ambiguous reply - here it silently reused
   the product with a full re-answer).
3. `stock?` -> **again the same stock reply for SRTWT2635**, not a clarifying question:
   `"Stock details found for the requested products.\n\n1. *Product Code:* SRTWT2635\n
   *Warehouse:* BUKIT RAJA\n*System Location:* BRW-SMC\n*Quantity On Hand:* 3\n*Outstanding:* 3\n
   \n_Data last updated: 04/09/2026 17:30:59_"`. `focus.products` still SRTWT2635 (`set_at_turn`
   still 2), `open_question: null`.

**Result: FAIL against the coordinator's expectation.** Under the default v13 prompt, the third
reply does NOT ask which product - the bot never let go of SRTWT2635 across all three turns.
Screenshot: `14-console-v13-stock-question-no-reset-1280.png` (the third turn, still answering for
SRTWT2635 instead of asking).

### Run 2 - Prompt version switched to v15

Same contact, session_vars carried over (still SRTWT2635 in focus from run 1 - harmless, turn 1
below re-sets it explicitly to the same value so the sequence starts from a known state).

1. `stock SRTWT2635` -> same stock reply as above, confirmed `prompt_version: 15`.
2. `another one` -> `"Could you clarify what you'd like to move to another one?"` -
   `session_vars.focus: null`, `open_question: null` - product AND domain both cleared in one
   shot (wording differs slightly from the previous session's v15 run - "Could you tell me what
   you'd like to switch to?" vs this run's "Could you clarify what you'd like to move to another
   one?" - live-LLM phrasing variance turn to turn, not a functional difference; the structural
   effect, a full focus clear, is identical both times).
3. `stock?` -> `"That would search every stock we have - I need at least one filter to narrow it
   down. Give me a product code or warehouse, and I can look it up."` - `focus.domains =
   ["inventory"]` (correctly read the domain word), no `focus.products`, `open_question: null`.
   Asks for the missing product rather than reusing the stale one.

**Result: PASS**, consistent with the prior session's v15 finding. Screenshot:
`15-console-v15-stock-question-asks-1280.png` (the third turn, asking for a filter instead of
answering).

### Verdict for the coordinator

**v13 (the production label) and v15 do NOT agree.** v13 still exhibits the pre-fix behaviour
(focus never clears on "another one", so "stock?" silently reuses the old product) even on
commit `dac7e946b`; v15 clears focus correctly and asks for the missing product on the third
turn, exactly as expected. Whatever `dac7e946b` fixed, it is not visible when the Console runs
against the v13 prompt template - worth confirming whether the fix was meant to apply
prompt-version-independently (in the deterministic `dialogue/focus.py` engine layer, per the
UAC's own framing of AC-1008/1003) or whether v13 itself needs the parser-prompt-side changes
(e.g. `topic_reset` emission) that v15 already carries. Console errors during this recheck
(verbatim): `Warning: Missing \`Description\` or \`aria-describedby={undefined}\` for
{DialogContent}.` only - the "key prop" list-warning from the earlier sessions did not reproduce
on this page in this run.

### Files (this recheck)

- `14-console-v13-stock-question-no-reset-1280.png` - v13, third turn, still answering SRTWT2635.
- `15-console-v15-stock-question-asks-1280.png` - v15, third turn, asks for the missing filter.
