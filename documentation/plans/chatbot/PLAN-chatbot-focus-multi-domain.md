...
| R-J: after a customer pick the scope question is PRINTED but not ARMED, so the next `1` re-picks the customer and the same question is printed again (owner smoke on c95f811bb, turns b4863e5d -> a509fbb0 -> add34025) | UNDER DIAGNOSIS. Measured on the live turns: the pick turn carries `open_question_answered: "customer_pick"`, prints "Outstanding for which document? 1/2/3", and persists `customer_pick`. The tail's ladder puts the ASK first and it wins wherever a test exercises it (probed across every outstanding shape in `test_outstanding_lane`: `asked = outstanding_scope` and it is what is persisted), so the precedence is not the cause on those shapes and the failing combination - a turn that ANSWERS a roster and composes the scope ask - is covered by no test today. Tester 2's red on the live chain is what pins which tail path discards it. | pending that red; no blind edit |
