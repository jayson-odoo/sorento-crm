...
| R-b `output_exchange` bare-code member-offer product narrowing (owner console pass 5, item B2, 7 Sep 2026, H81/AC-829) | the customer's bare reply, tokenised (`_TOKEN_RE`) to ONE word, compared by EQUALITY against a SEGMENT of a code WE persisted on `prev_state.routing_companies[].codes` (itself tokenised the same way) - never against the code's raw text as a whole, and never a substring or regex scan of the reply | a read of the customer's message, same class as `_team_clarify_pick` directly above: equality against a closed set of strings this codebase itself computed by splitting our own persisted state, not a pattern match over prose. Fires only when the candidate code's FIRST segment equals the carried product entity's own token (`"srtwc286"` for `"SRTWC286-SH-RPACC"`), which is what tells a product variant apart from the customer's own debtor code sitting in the same `codes` list (gate.py's `_co_label` comment: `codes` mixes both). "rpacc" (turn 6ea9fd1a) is the capture; `entities: []` this turn, so nothing downstream of the parser can invent the narrowing without a read here. Delete it if `routing_companies` ever stops carrying product-variant codes alongside debtor codes |
...
| H79 | a migration seeding a new `attachment_types` row broke "photo" resolution | **fixed (AC-827, console pass 5, 7 Sep 2026)** - prod turns a5317cf4 / b4369aba, three minutes after control turn 008676fc sent the file clean. The deploy that shipped #713 (043e2a0be) also carried #707's migration `485_shipment_line_photo_type.py`, seeding "Shipment Line Photo" (`entity_type='inbound_shipment_line'` on every attachment it is ever used with - `app/services/scm/shipment_line_photos.py` - never `product`) alongside the pre-existing "Product Photos" row; both contain the substring "photo", so Tier 2's substring probe now returns two candidates where it used to return one. `gate.py`'s own non-product ambiguity handling (`run_gate`'s OR-mode `non_products` branch) has no per-type narrowing and silently picked `non_products[0]`, order-dependent - a wrong silent pick, or (once that pick's own fetch came up empty) a customer-facing did-you-mean, never a deterministic delivery of the file that DOES exist. #713's own diff never touches attachment-type resolution (`git diff 5e8acfef4..043e2a0be` confirms), so the migration is the whole cause. Fixed by DOMAIN-SCOPING at the resolver, not by touching the shared non-product ambiguity handler (which every other entity type funnels through too, and changing it is a much wider, unmeasured blast radius): `entity_resolver._product_attachment_type_ids` restricts Tier 2 `attachment_type` candidates, ONLY when `domain_hint == "product_attachment"`, to types at least one PRODUCT-entity attachment actually carries - measured against the `attachments` table itself, not a hardcoded denylist of codes a future internal type could silently miss. Tier 1 exact stays untouched, so a customer who types the internal type's own exact code or name still resolves it. Two of the three committed reds are `xfail(strict=True)` instead of fixed here (tracked as issue #727): `product_attachment` has no deterministic fetch path that skips the MCP tool-search-then-call pipeline, and building one means duplicating or rerouting the access-level / company-scope logic `/api/v1/master-data/product-attachments` already carries - a materially larger, security-relevant change this item's own scope never asked for. Evidence: `tests/chatbot/test_pass5_item1_photo_attachment_alias.py`. |
| H80 | a "last month" that set a concrete date was wiped by its own `broaden_axis` | **fixed (AC-828, console pass 5, 7 Sep 2026)** - prod turns e4381b0d / 98526b81 (member_offer, business_query, `date_filter_start/end` set by the SAME parser output that also carries `broaden_axis: "date"`). `head/output_exchange.py`'s `entity_op == "reuse"` arm treats `broaden_axis == "date"` as an unconditional "drop the window" signal and nulls `date_filter_start` / `date_filter_end` / `date_mode` before ever consulting `has_current_date` (computed one line above, read only by the sibling `elif`), so a turn that named `broaden_axis: "date"` AND supplied a real date was wiped exactly like a turn that said "all dates" and supplied none. Pre-existing, not part of #713's diff (confirmed by line range). Fixed by gating the wipe on `not has_current_date` too - a turn that supplies a concrete date this turn keeps it, whatever `broaden_axis` says. Evidence: `tests/chatbot/test_pass5_item2_member_offer_business_query_filter_route.py::TestB1LastMonthUnderMemberOfferKeepsTheParsersOwnDateFilter`. |
| H81 | a bare product code under an open member offer had no narrowing mechanism | **fixed (AC-829, console pass 5, 7 Sep 2026, D11 inventory row R-b)** - prod turn 6ea9fd1a: "rpacc" after a working "last month" (offer hanlim/srtwc286) is never extracted as an entity by the parser at all (`entities: []`, `message_type: "casual"`), so nothing downstream of the parser could invent a product entity for it, and the reply kept naming the OLD product ("Product: srtwc286") instead of narrowing to the new one. A genuine gap, not a wrong branch - no site read a bare reply against `prev_state.routing_companies[].codes` (where product variant codes like "SRTWC286-SH-RPACC" live) to substitute the product half of a carried entity pair. Fixed with a NEW deterministic reader, D11-compliant by construction: the bare reply, tokenised to ONE word, is compared by EQUALITY against a SEGMENT of a code persisted on `routing_companies[].codes` (itself tokenised the same way) - never a substring or regex scan of the reply - and the candidate code's FIRST segment must equal the carried product entity's own token, which is what tells a product variant apart from the customer's own debtor code sitting in the same list. Also fixes a second-order defect the mechanism exposed: the newly-narrowed entity's `current_message: True` left `message_type` at "casual", which routed the REST of the turn (the tail's `escalate-catalog` / `cs-offer-gate`) as an unanswered offer even though the query had just been answered - `message_type` now promotes to "business_query" the same way the pre-existing "bare entity under an open member roster" block (AC-816 rule 3) already does. Evidence: `tests/chatbot/test_pass5_item2_member_offer_business_query_filter_route.py::TestB2ABareProductCodeUnderTheOfferNarrowsTheProduct`. |
