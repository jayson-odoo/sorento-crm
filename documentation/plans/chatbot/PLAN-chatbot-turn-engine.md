...
| H58 | the MCP tool pick could select a WRITE tool | **fixed (AC-813, this PR)** - the business lane picks ONE tool per turn by cosine similarity and calls the single top hit (`lanes/business/fetch.py::tool_filter`) with no allow-list check, and the embedded catalogue contained `crm_it_support_ticket_create`, `crm_complaint_close`, `crm_order_cancel`, `crm_purchase_request_approve` and `crm_purchase_request_reject`. Nothing but the scores stood between an ordinary customer question and cancelling an order. Fixed in BOTH layers, from ONE source of truth (`sorento_crm_mcp/catalog.py`'s own declaration, read by `mcp_tool_capability_service.read_only_tool_names`): a tool READS when its method is GET or its spec sets `read_only=True`, and everything else is excluded from the embedding pool so it is never a candidate, AND `fetch.call_tool` refuses it at the egress with the `tool_not_allowed` fetch outcome. The flag exists because METHOD IS TRANSPORT, NOT SEMANTICS: `crm_lookup_resolve`, `crm_portal_link_get` and `user_guides_read` are POST only because their input does not fit in a query string, they write nothing, and dropping them would have taken three useful reads out of the AI assistant's tool search, which shares this pool. Its default (False) is what keeps a real write tool out by construction. Two layers because the pool is DATA already written on a live install (it shrinks only when the seed is re-run with `--rebuild`) while the refusal is code that ships with the deploy. Evidence: `tests/chatbot/test_tool_pool_is_read_only.py` (13 tests) and `test_dry_run_isolation.py::TestMcpToolPickRefusesWriteTools`. |
