...
| H67 | the ambiguous-customer picker could not be told apart, and its "has delivery" stamp counted any order | **fixed (AC-817, this PR)** - two defects on one line. The line rendered the account name alone, and `_repLabel` deletes exactly the bracketed marker that distinguishes one company ledger's copy of a name from another's, so "A CRAFT IDEA SDN BHD" appeared twice and the pick was a coin toss; the line now carries the owning company's `companies.code` (`(SRT)`, `(MOCHA)`), which the resolver stamps as `company_code` beside `company_name` in `_attach_company_info`. And `pickers.annotate_customer`'s `with_delivery` set was built from EVERY row `crm_order_management_orders_list` returned, which answers "does this customer have ORDERS", not "does this customer have a DO" - a customer whose orders had not shipped read `- has delivery`. It now counts only rows carrying an `Actual Delivery Date` and the stamp reads `- has DO` / `- no DO`. Registered field-scoped on the 4 `annotate-customer-picker` captures and the 2 `sub-resolve-and-gate` exit arms that carry the message; none of it can be fixture-visible, because n8n's own body says "delivery" and counts every row. Evidence: `tests/chatbot/test_s6a_gate_dry_run_and_seams.py::TestOwnerRulingACustomerPickerDOStamp`, `tests/chatbot/test_resolve_gate_unit.py::TestPickerProbeArms::test_an_order_with_no_delivery_order_is_not_counted_as_one`. |
