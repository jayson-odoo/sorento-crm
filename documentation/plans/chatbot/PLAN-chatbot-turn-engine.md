...
| H77 | a picked customer was re-resolved from a debtor code 99 accounts share | **OPEN, diagnosed and pinned (AC-825, this PR)** - turns 5bb0426e -> cc0075ae / 0d9332a0. Measured against the local prod-copy DB: the date window survives every stage and the resolver mis-ranks nothing (`gate_passed: true`, `require_specific: false`, full coverage), so neither first-pass hypothesis holds. `resolve_entity_body` sends the carried entity's `canonical_code` as the token and omits `entity_pins`, because the resolver route refuses a pin in AND mode by design (`references.py` ~:1652; PR #456 is OR-mode only) - so the uuid the pick IS gets thrown away and a code 99 production rows share is re-resolved, returning 15 other accounts without the picked one. The gate then passes them: `cust_pinned` suppresses the "which company" ask, so the pin stops the QUESTION without constraining the ANSWER. NOT fixed here: the fix is AND-mode pins in `resolve_references`, that route's own contract - and not a lane waiting to merge, since PR #456 is MERGED (9f04a3205) and is what AUTHORED the refusal, deliberately (no per-token view in an intersection, and a zero-intersection retry under `force_mode="or"` where a pin would suddenly apply). Tracked as issue #715. Committed as `xfail(strict=True)` so the fix announces itself; the end-to-end guard beside it is green because below production scale the code is unambiguous and the defect cannot exist. Evidence: `tests/chatbot/test_pass4_item2_last_month_keeps_customer_scope.py`. |
