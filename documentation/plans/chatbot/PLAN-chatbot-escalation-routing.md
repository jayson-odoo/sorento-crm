...
**Finding 6 (D10, captain's console re-pass, 13 Sep 2026, prompt v16): the brand was still
blank on a resumed team_pick, live and dry alike.** `ESCLATE TO MARKETING ABOUT WATER TAP OF
SRTWB8004` resolved the product (D6) then asked which marketing team (D2); the `team_pick`
persisted `payload: {team: "purchasing", domain: "master_products"}` (the INHERITED team,
`routing_source: none`), with `focus.domains: master_products` and `focus.products:
[SRTWB8004, canonical_code null]`. "MARKETING PRODUCT" answered it
(`ctx.parse._answered.handler == "team_pick"`) and landed `marketing_product` - but
`_this_turn_products` is empty (SRTWB8004 carries `current_message: false` on this turn), so
`_resolve_product` returns `None`, and `_carried_brand`'s D3 same-team gate compared the
LANDED team (`marketing_product`) against `_carried_team(ctx)` (`purchasing`, derived from
`focus.domains`) and returned `(None, None)` - the copy read "...from Marketing Product
...
- **Reviewer N3, review round 10 (`tail/compile_state.py` ~2316, `_miss_company_routing`'s
  Case A re-persist branch).** When a member-offer (`build-miss-member-offer`) clarify
  fires on the SAME turn a `team_pick` was already open, that branch rebuilds the
  persisted marker for the NEW offer without carrying the old `team_pick`'s own fields
  forward - `turn_state["team_clarify_options"]`, the asked-at clock, and now D10's
  `team_pick_product_code` are all lost, empty rather than frozen. Latent, not built
  around: it is UNREACHABLE from this lane, because `escalation._clarify_gate` only
  diverts to the company-clarify arm on `multi_company_unpicked`, never on a
  `member_offer`, so an escalation `team_pick` and a miss-lane member offer cannot both be
  open going into the same turn today. Left as a shared-path trap for whichever lane next
  arms a `team_pick` alongside a member offer to measure and fix, not a defect this lane's
  own UAC claims coverage of.
