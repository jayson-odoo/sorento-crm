...
| D32 | Create a packing list for a container the system does not have? | **No. Update-only.** REVISED after running the import for real. The first version created one, and on the live workbook that produced **296 hollow rows** - no shipment number, no supplier, no lines, a guessed `shipment_date` - sitting in the Packing Lists grid with "-" in every column, indistinguishable at a glance from real packing lists. The sheet has no lines, products, quantities or supplier, so a row for an unknown container simply is not a packing list. A packing list comes from an actual packing list; this sheet annotates the ones that exist. Unmatched rows are skipped, counted, sampled in the dry-run warning and written to Import Job Details. **Consequence accepted:** coverage is 111 of 407 containers (27%) until the rest have packing lists - and that number must stay visible rather than be hidden by inventing rows to fill it. |
| D33 | One current status, or a timeline of checkpoints? | **Timeline of checkpoints.** A container does not walk a straight line: the real workbook has containers with a gatepass and no inspection, an ETA delay and no ETA, a collection with no warehouse arrival. A single `status_id` would force us either to invent a position the sheet never recorded, or to imply that every earlier stage completed. So each checkpoint is independently reached or not, all eleven are shown, and there is **deliberately no `status_id` column on `inbound_shipments`** and no `status_transitions` - nothing auto-advances, a checkpoint is reached because its date column is filled. The status engine is still adopted, but for the CONFIGURATION rather than the position: which checkpoints exist, their names, captions, grouping, order, colour and visibility. `statuses.key` is frozen on update by the API, which is what makes it safe as the column contract - an admin can rename "Gatepass" to "Released from port" without breaking the link to `gatepass_date`. `migrate_records` is a no-op because moving every container's gatepass onto its inspection date would be corruption, not a migration; `count_records` reports how many containers reached a checkpoint, which is what blocks deleting one that is in use. The four unmaintained fields (ATA, ORI DOC, K1, YARD - filled 6/4/4/4 of 407) are **not** checkpoints: four permanently grey dots would drown the eleven that matter. |
| D34 | Keep ATA / ORI DOC / K1 SUBMISSION / YARD ARRIVALS? | **No. Removed from the system entirely** - not columns, not parsed, not shown. They are on every tab but filled 6 / 4 / 4 / 4 times across 407 containers, and nothing reads them: no status node, no alert, no integration. An earlier version stored them "for round-tripping" behind a muted "rarely filled" block, which is a column nobody maintains plus a UI nobody reads. The retained original file preserves the history if anyone ever needs it (same argument as the deferred cost block, D9). |
...
**S2 - status engine adoption, as a CHECKPOINT TIMELINE.** `inbound_shipment` becomes the **first
registered status entity in this repo** - `register_status_entity` with `count_records` (containers
that reached a checkpoint) and `migrate_records` (a no-op, see D33). The eleven milestones are seeded
as `statuses` rows: `key` = the date column, `label` / `description` / `category` / `sort_order` /
`color_hex` / `is_active` all admin-editable in System Management -> Status Graphs. **No `status_id`
column and no `status_transitions`** - see D33. The frontend renders the timeline from
`GET /packing-lists/clearance-checkpoints`, so adding, renaming, reordering or hiding a checkpoint is
an admin action rather than a release.

**S3 - importer.** New container-status import on the `imports` queue. **Update-only (D32)** - own
matcher (normalized container, all statuses - explicitly not the packing-list matcher's
not-fully-received rule); unmatched rows skipped, counted and reported, never created.
