...
**Architecture:** Phase 3 of `documentation/superpowers/specs/2026-05-01-mcp-tool-access-guard-design.md` (§7, §8) plus admin UI for configuration visibility. Decision logic from §8: `tool→owner agent→ON owner.is_active→contact lookup→ContactAgentAccess match`. Five decision branches all log to `mcp_access_log`. MCP server uses an asyncio-locked TTL dict keyed on `(tool_name, contact_id, space_id)`.
